...
- **Live preview** — lightweight client-side parse shows detected entries before submit (comma-grouped amounts like `40,200/-` included)
- **Connections never become destinations** — a header like `CCJ - MCT – DXB` is a through-fare to Dubai via Muscat, so both the preview and the n8n parser record the sector as `CCJ-DXB`. First airport is the origin, last is the destination, everything between is a change of plane. See the gotcha in [AGENTS.md](AGENTS.md).
