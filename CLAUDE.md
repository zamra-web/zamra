...
**Security rules** ([firestore.rules](firestore.rules)): almost every content collection is world-readable with admin-only writes; `agent_fares` is readable when `isHidden == false`; `b2b_agents` is readable only by an admin or the agent whose `b2bAgentId` matches. Known gap flagged for follow-up: raw `specialRate`/`commission` on `agent_fares` are publicly readable.
