...
- `ingestFaresFromN8n` / `exportFlightDetailsForN8n` are `onRequest` endpoints for the n8n integration, secured by Bearer token. Ingest **appends** a new auto-id row per fare and never edits one, but it now hides the rows an upload replaces first (`functions/fareSupersede.js`, keyed `agentId + sectorId + airlineId + flightDate + flightTime`). Both halves are load-bearing: every projection dedupes that group by *minimum price*, so without the supersede an upward revision would be ignored forever in favour of the stale cheaper row. Never widen that key past the supplier — it is what stops one sheet delisting a competitor's quote. The same file also reads **absence** as a sold-out signal (`planAbsenceSoldOut`) — a flight dropped from a supplier's newest complete sheet, which is how most suppliers say "sold out". That one hides live fares on an inference, so it is off by default at three levels (global config, per-supplier `agents.rateIntakeAbsenceSoldOut`, and an allow-list of upload sources that excludes `portal`) behind three fail-closed guards. **Read the "Absence on ingest" section in [DASHBOARD.md](DASHBOARD.md) before touching it** — every guard exists because removing it silently delists fares that are still for sale.
