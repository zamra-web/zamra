...
- WhatsApp (WAHA): `functions/whatsapp/` proxies a self-hosted [WAHA](https://github.com/devlikeapro/waha) instance running beside n8n on the Hostinger VPS (see [infra/README.md](infra/README.md)). The admin callables are a **whitelist that projects every response** — `GET /api/sessions` returns the session config, which contains our own webhook HMAC key. `whatsappWebhook` verifies HMAC-SHA512 over `req.rawBody` and mirrors messages to `whatsapp_messages` / `whatsapp_chats`. Messaging logic (auto-replies, broadcasts) lives in n8n, not here.
