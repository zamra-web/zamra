...
- `ingestFaresFromN8n` / `exportFlightDetailsForN8n` are `onRequest` endpoints for the n8n integration, secured by Bearer token. Ingest **appends** a new auto-id row per fare and never edits one, but it **deletes** the rows an upload replaces (`functions/fareSupersede.js`, keyed `agentId + sectorId + airlineId + flightDate + flightTime`). Because the replaced row was the older sibling `annotateFarePriceDrops` derived a re-upload price drop from, ingest stamps the old price onto the cheaper new row as `previousFinalRate` — change one, check the other. Both halves are load-bearing: every projection dedupes that group by *minimum price*, so without the supersede an upward revision would be ignored forever in favour of the stale cheaper row. Never widen that key past the supplier — it is what stops one sheet delisting a competitor's quote. The same file also reads **absence** as a sold-out signal (`planAbsenceSoldOut`) — a flight dropped from a supplier's newest complete sheet, which is how most suppliers say "sold out". That one **hard-deletes** live fares on an inference — as do the other two removal paths, the explicit "SOLD OUT" WhatsApp notice (`applySoldOut` in `functions/whatsapp/rateIntake.js`) and the supersede above; nothing in the ingest path hides any more, and every deleted row is named in Cloud Logging because that is the only trail left — so it is off by default at three levels (global config, per-supplier `agents.rateIntakeAbsenceSoldOut`, and an allow-list of upload sources that excludes `portal`) behind three fail-closed guards. **Read the "Absence on ingest" section in [DASHBOARD.md](DASHBOARD.md) before touching it** — every guard exists because removing it silently deletes fares that are still for sale, with no un-hide to recover them.
- WhatsApp (WAHA): `functions/whatsapp/` proxies a self-hosted [WAHA](https://github.com/devlikeapro/waha) instance running beside n8n on the Hostinger VPS (see [infra/README.md](infra/README.md)). The admin callables are a **whitelist that projects every response** — `GET /api/sessions` returns the session config, which contains our own webhook HMAC key. `whatsappWebhook` verifies HMAC-SHA512 over `req.rawBody` and mirrors messages to `whatsapp_messages` / `whatsapp_chats`. Messaging logic (auto-replies, broadcasts) lives in n8n, not here. `whatsappRateIntakeForN8n` is the one exception in spirit: it leases batches of a linked supplier's inbound messages so n8n can feed them to the **existing** `zamra-rates` extraction workflow, populating `agent_fares` with no human step. It ships behind `config/whatsapp.rateIntakeEnabled` (false) and a per-supplier `agents.rateIntakeMode` (off). Suppliers who broadcast to a **community announcement group** are ingested too, but only from `agents.rateIntakeGroupIds` (which doubles as the mirror allow-list, so `mirrorGroups` stays false) and only from a sender matching `whatsappChatId` or `rateIntakeSenderIds` — membership is not authorship, and every ambiguous case fails closed.
