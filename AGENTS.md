...
- **WAHA's NOWEB engine does not speak `@c.us`.** It reports direct chats as `<number>@s.whatsapp.net`, and WhatsApp now addresses many contacts by **LID** — `224876132614243@lid`, an opaque id that is not a phone number. The real number rides in `payload._data.key.remoteJidAlt`. Accepting only `@c.us` made `normalizeChatId` throw, `isMirrorableEvent` return false, and every inbound message get dropped with a **200 and no error anywhere** — the integration looked healthy and mirrored nothing for days. `chatIdFromPayload` swaps in `remoteJidAlt` only when the primary id is a LID: in a group that field is the individual sender, and swapping there would file group traffic under a person.
- **`MAX_TEXT_LENGTH` is the OUTBOUND send limit, not an inbound one.** A supplier's daily rate sheet runs past 6,900 characters — forty sectors — and slicing inbound bodies at 4096 dropped the last third while the batch still reported success. `MAX_INBOUND_TEXT_LENGTH` bounds the mirror; `sendGuard` keeps 4096 because WhatsApp really does enforce it on send.
...
- **`tableLimit` defaults must match an option in the matching `<select>`** — `databaseFares` defaulted to `25` while `#database-limit` offered only 20/50/100/250 at the time, so the select rendered blank on first paint. Fixed to `20`; the select now offers 20/50/100/250/500. Keep them in step.
- **A re-render must not cost the admin their place** — every save ends in a re-render, and a re-render used to lose the row being edited two ways. Scroll: a browser clamps a scroll offset the moment the content behind it is too short to hold it and never gives it back when the content grows again (Chrome, measured: a 5000px page shrunk to 600px while scrolled to 1200 reports scrollY 147 from then on), so any repaint that is even briefly shorter spends the position — the window and `.admin-database-wrap` (`overflow: auto`, `max-height: 72vh`) alike. Pagination: each `renderXxxTab()` refetch reset `tablePage` to 1, so editing row 4 of page 7 landed on page 1. Fix: every renderer that repaints an editable table wraps its body in **`keepPlace()`** (`renderXxxTab` → `renderXxxTabInner`), and the `if (fetchData)` blocks leave `tablePage` alone — the search/limit/filter handlers still reset it, and each renderer clamps to the last page anyway. `web/tests/admin-keeps-place.test.js` fails if a renderer or a refetch drops back out of the pattern.
...
- **A connection is not a destination.** Suppliers print the journey, not the sector: `CCJ - MCT – DXB (30+7kg)` is one Calicut→Dubai fare changing planes in Muscat. Read as "the first two codes" it becomes `CCJ MCT` — a Dubai price on a Muscat seat, and since `CCJ MCT` is usually a real sellable sector, neither the closed vocabulary nor the rate band catches it. The rule is: **first airport = origin, last airport = destination, everything between is a connection** (`CCJ-DXB via MCT` says the same thing; `CCJ/COK - DXB` does not — a slash is alternative origins, not a stop). It is enforced in three places that must stay in step — the system prompt in `Build Vision Request`, the `route_text` guard in `Build Firebase Payload` (both in [n8n/zamra-rates.workflow.json](n8n/zamra-rates.workflow.json), tested by `functions/tests/n8n-workflow.test.js`), and [web/src/js/shared/rate-route.js](web/src/js/shared/rate-route.js) for the upload preview (tested by `web/tests/rate-route.test.js`). The n8n copy cannot import the shared module — that Code node runs in n8n's sandbox — so the duplication is deliberate.
