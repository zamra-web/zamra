...
- **Poster social publishing is market-driven** — the admin poster page now has a compact Social Publishing strip for Saudi, UAE, Bahrain, Oman, Kuwait, and Qatar. Selecting a market reveals the Images/Videos queue actions, and they use the selected date range, not the current preview stack or sector dropdown.
- **Social market matching is India ↔ market only** — poster social batching should only include sectors where one side is one of the India airports (`CCJ`, `COK`, `CNN`, `TRV`, `IXE`) and the other side is one of the configured market airports. Do not include market ↔ market routes in those batches.
- **Buffer posting is per-market now** — `social_queue` docs must carry `marketKey`, and the Cloud Function resolves market-specific Buffer secrets/channel IDs from code. The placeholder channel IDs in `functions/buffer/marketConfig.js` must be replaced with the real Buffer channel IDs before live posting can succeed.
