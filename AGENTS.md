...
- **Poster video slideshow** — if a route spans multiple pages, the video export merges them into one slideshow (same animation style per page). Do not re‑introduce per‑page video exports for the same route.
...
- **Poster social publishing is airport-group driven** — the admin poster page now has a compact Social Publishing strip for `Calicut (CCJ)`, `Kochi (COK)`, `Kannur (CNN)`, `Trivandrum (TRV)`, and `Mangalore (IXE)`. Selecting an airport group reveals the Images/Videos queue actions, and they use the selected date range, not the current preview stack or sector dropdown.
- **Social airport matching is India-side only** — poster social batching should only include sectors where exactly one side is one of the India airports (`CCJ`, `COK`, `CNN`, `TRV`, `IXE`). The publishing bucket must always be the India-side airport, regardless of direction, and non-India routes must stay excluded.
- **Buffer posting is per-airport now** — `social_queue` docs must carry `marketKey`, and the Cloud Function resolves airport-specific Buffer secrets/channel IDs from code. The placeholder channel IDs in `functions/buffer/marketConfig.js` plus the Firebase secrets `BUFFER_API_KEY_CCJ`, `BUFFER_API_KEY_COK`, `BUFFER_API_KEY_CNN`, `BUFFER_API_KEY_TRV`, and `BUFFER_API_KEY_IXE` must be replaced with real values before live posting can succeed.
...
- **Airport image posting is country-carousel based** — a single airport Images click should create up to 6 country carousels only (`Saudi`, `UAE`, `Oman`, `Qatar`, `Bahrain`, `Kuwait`), each capped at 10 images, so one airport run stays at a maximum of `12` Buffer `createPost` calls.
