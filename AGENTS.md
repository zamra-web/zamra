...
- **`buildFlightCardHtml` does not escape, `buildSotoCardHtml` must** — the shared flight card interpolates every value raw, which is safe only because its inputs are admin-controlled Firestore rows. SOTO data comes from a third-party API, so every field goes through `escapeHtml()`. Do not "unify" the two card builders without fixing the escaping first.
