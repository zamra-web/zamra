import"./firebase-config-zYKzaodH.js";import{g as X,a as q,b as W}from"./db-BxSq6g0-.js";document.addEventListener("DOMContentLoaded",()=>{const e=document.getElementById("header");window.addEventListener("scroll",()=>{window.scrollY>50?e.classList.add("scrolled"):e.classList.remove("scrolled")});const t=document.getElementById("mobile-toggle"),n=document.getElementById("nav-menu");t&&t.addEventListener("click",()=>{n.classList.toggle("active");const l=t.querySelector("i");n.classList.contains("active")?l.classList.replace("bi-list","bi-x-lg"):l.classList.replace("bi-x-lg","bi-list")}),document.querySelectorAll('a[href^="#"]').forEach(l=>{l.addEventListener("click",function(C){const w=this.getAttribute("href");if(n&&n.classList.contains("active")&&(n.classList.remove("active"),t&&t.querySelector("i").classList.replace("bi-x-lg","bi-list")),w&&w!=="#"){const h=document.querySelector(w);if(h){C.preventDefault();const v=h.getBoundingClientRect().top+window.pageYOffset-80;window.scrollTo({top:v,behavior:"smooth"}),window.history.pushState(null,"",w)}}})});const g=[{id:"kozhikode",code:"CCJ",name:"Kozhikode"},{id:"kochi",code:"COK",name:"Kochi"},{id:"kannur",code:"CNN",name:"Kannur"},{id:"trivandrum",code:"TRV",name:"Trivandrum"},{id:"mangalore",code:"IXE",name:"Mangalore"}],f=[{id:"jeddah",code:"JED",name:"Jeddah"},{id:"riyadh",code:"RUH",name:"Riyadh"},{id:"dammam",code:"DMM",name:"Dammam"},{id:"doha",code:"DOH",name:"Doha"},{id:"muscat",code:"MCT",name:"Muscat"},{id:"bahrain",code:"BAH",name:"Bahrain"},{id:"kuwait",code:"KWI",name:"Kuwait"},{id:"dubai",code:"DXB",name:"Dubai"},{id:"sharjah",code:"SHJ",name:"Sharjah"},{id:"abudhabi",code:"AUH",name:"Abu Dhabi"},{id:"rasalkhaimah",code:"RKT",name:"Ras Al Khaimah"},{id:"alain",code:"AAN",name:"Al Ain"},{id:"fujairah",code:"FJR",name:"Fujairah"}],b=document.getElementById("flight-grids-container");if(b){const l=(C,w,h)=>{const E=document.createElement("div");E.className="mb-[50px]",E.innerHTML=`
      ...
      `,b.appendChild(E);const N=E.querySelector(`#grid-${h.replace(/\s+/g,"-").toLowerCase()}`);C.forEach(v=>{const S=document.createElement("div");S.className="sector-card bg-gradient-to-r from-primary to-[#1558c0] p-[18px_24px] rounded-[16px] shadow-[var(--shadow-premium-soft)] cursor-pointer hover:shadow-[0_8px_25px_rgba(26,115,232,0.3)] hover:-translate-y-1 transition-all duration-300 flex items-center relative overflow-hidden group",S.innerHTML=`<h4 class="text-[17px] font-heading font-extrabold text-white m-0 flex items-center justify-between z-[2] relative w-full">${v.name} (${v.code}) <i class="bi bi-arrow-right-circle text-white/80 text-[22px]"></i></h4>`,S.addEventListener("click",()=>{typeof i=="function"&&i(v,w)}),N.appendChild(S)})};l(g,f,"India"),l(f,g,"Middle East")}const r=document.getElementById("sector-modal"),x=document.getElementById("modal-close"),d=document.getElementById("modal-body"),c=document.getElementById("modal-route"),k=document.getElementById("modal-title");function i(l,C){k.textContent="Select Destination",c.textContent=`Flying from ${l.name}`,c.classList.remove("bg-primary-light","text-primary"),c.classList.add("bg-slate-100","text-slate-600"),r.classList.add("active"),document.body.style.overflow="hidden",d.innerHTML=`
    ...
    `;const w=document.getElementById("routes-grid");C.forEach(h=>{const E=`${l.code} ${h.code}`,N=`${l.name} → ${h.name}`,v=document.createElement("button");v.className="bg-white p-4 rounded-xl border border-border shadow-sm hover:shadow-md hover:border-primary transition-all flex items-center justify-between group cursor-pointer w-full text-left",v.innerHTML=`
      ...
      `,v.onclick=()=>{A(E,N)},w.appendChild(v)})}function A(l,C){k.textContent="Flight Details",c.textContent=l.replace(" "," → "),c.classList.add("bg-primary-light","text-primary"),c.classList.remove("bg-slate-100","text-slate-600"),r.classList.add("active"),document.body.style.overflow="hidden",d.innerHTML='<div class="w-[40px] h-[40px] border-[3px] border-[#f3f3f3] border-t-primary rounded-full animate-spin mx-auto my-[30px]"></div><p class="text-center text-text-muted mt-4">Fetching latest fares...</p>';async function w(){try{const E=(await X()).find(I=>I.sectorCode===l),N=await W(),v={};N.forEach(I=>v[I.id]=I.name);let S="";if(E){const I=new Date;I.setHours(0,0,0,0);let M=await q({sectorId:E.id,startDate:I.toISOString()});M.sort((p,$)=>p.flightDate.getTime()===$.flightDate.getTime()?p.finalRate-$.finalRate:p.flightDate.getTime()-$.flightDate.getTime()),M.length===0?S='<tr><td colspan="5" class="p-[14px_15px] text-center text-text-muted">No flights available currently.</td></tr>':S=M.map(p=>{const $=v[p.airlineId]||"Unknown Airline",Q={day:"2-digit",month:"short",year:"numeric"},tt=p.flightDate.toLocaleDateString("en-GB",Q),et=p.flightTime&&p.flightTime.split("-")[0]?p.flightTime.split("-")[0].trim():"TBA",it=p.flightTime&&p.flightTime.includes("-")?p.flightTime.split("-")[1].trim():"TBA";return`
                          <tr class="border-b border-[#e2e8f0] [&:nth-of-type(even)]:bg-[#fafbfc] [&:last-of-type]:border-b-2 [&:last-of-type]:border-primary hover:bg-[#f1f5f9] transition-colors">
                              ...
                              <td class="p-[14px_15px] whitespace-nowrap"><strong>${$}</strong></td>
                          ...
                          </tr>`}).join("")}else S='<tr><td colspan="5" class="p-[14px_15px] text-center text-text-muted">No flights available currently.</td></tr>';d.innerHTML=`
                <div class="text-center mb-4">
                    ...
                    <h4 class="text-primary-dark font-bold text-lg mb-[8px]">Available Flights for ${C}</h4>
            ...
            `;const J=document.getElementById("back-to-routes");J&&J.addEventListener("click",()=>{const I=l.split(" ")[0];let M=g.find($=>$.code===I),p=f;M||(M=f.find($=>$.code===I),p=g),M?i(M,p):y()})}catch(h){console.error("Error fetching fares:",h),d.innerHTML='<p class="text-center text-red-500 my-4">Error loading flights. Please try again later.</p>'}}w()}function y(){r.classList.remove("active"),document.body.style.overflow=""}x&&x.addEventListener("click",y),r&&r.addEventListener("click",l=>{l.target===r&&y()}),document.addEventListener("keydown",l=>{l.key==="Escape"&&r.classList.contains("active")&&y()}),(()=>{document.querySelector(".partners-slider")})();const o=document.getElementById("live-search-btn");o&&o.addEventListener("click",()=>{typeof F=="function"&&F()});const s=document.getElementById("swap-locations"),m=document.getElementById("swap-locations-mobile"),u=document.getElementById("origin"),B=document.getElementById("destination"),z=()=>{if(u&&B){const l=u.value;u.value=B.value,B.value=l}};s&&s.addEventListener("click",z),m&&m.addEventListener("click",z)});const nt="https://n8n.srv1046139.hstgr.cloud/webhook/zamra",st=[1,2,21,22,23,24,25,28,31,40,42,43,44];let T=null,L=JSON.parse(localStorage.getItem("zt_hist")||"[]"),G=L.reduce((e,t)=>e+(t.rows||0),0);function U(){document.getElementById("statSubs").textContent=L.length,document.getElementById("statEntries").textContent=G}function ot(){const e=document.getElementById("chipGrid");!e||e.children.length>0||st.forEach(t=>{const n=document.createElement("div");n.className="rp-chip",n.textContent=t,n.style.cssText="height:48px;display:flex;align-items:center;justify-content:center;border:2px solid #b8cce4;border-radius:10px;font-size:15px;font-weight:700;color:#1e293b;cursor:pointer;background:#ffffff;user-select:none;box-shadow:0 1px 4px rgba(13,31,60,.10);transition:all .16s ease;",n.addEventListener("click",()=>at(t,n)),e.appendChild(n)})}document.addEventListener("DOMContentLoaded",()=>{ot(),O(),U()});function at(e,t){T=e,document.getElementById("manualAgent").value="",document.querySelectorAll(".rp-chip").forEach(n=>{n.classList.remove("on"),n.style.background="#ffffff",n.style.color="#1e293b",n.style.borderColor="#b8cce4",n.style.boxShadow="0 1px 4px rgba(13,31,60,.10)",n.style.transform=""}),t&&(t.classList.add("on"),t.style.background="#1a73e8",t.style.color="#ffffff",t.style.borderColor="#1a73e8",t.style.boxShadow="0 4px 14px rgba(26,115,232,.3)",t.style.transform="translateY(-1px)"),V(),R()}document.getElementById("manualAgent").addEventListener("input",function(){const e=parseInt(this.value);T=e>0?e:null,document.querySelectorAll(".rp-chip").forEach(t=>{t.classList.remove("on"),t.style.background="#ffffff",t.style.color="#1e293b",t.style.borderColor="#b8cce4",t.style.boxShadow="0 1px 4px rgba(13,31,60,.10)",t.style.transform=""}),V(),R()});function V(){const e=document.getElementById("agentPill");T?(e.textContent=`Agent ${T} selected ✓`,e.classList.remove("empty")):(e.textContent="No agent selected",e.classList.add("empty"))}const D=document.getElementById("rateData");let K;D.addEventListener("input",function(){const e=this.value.length;document.getElementById("charCount").textContent=e.toLocaleString()+" character"+(e!==1?"s":""),R(),clearTimeout(K),e>15?K=setTimeout(()=>rt(this.value),500):j()});function R(){document.getElementById("submitBtn").disabled=!(T&&D.value.trim().length>10)}function Z(e){const t=[],n={JAN:"01",FEB:"02",MAR:"03",APR:"04",MAY:"05",JUN:"06",JUL:"07",AUG:"08",SEP:"09",OCT:"10",NOV:"11",DEC:"12"},a=/\b(IX|6E|G9|SV|WY|XY|QP|FZ|OV|AI|J9|SG)\b/;let g=null,f="IX";for(const b of e.split(`
`)){const r=b.replace(/[*_~`]/g,"").trim();if(!r)continue;const x=r.match(/([A-Z]{3})\s+([A-Z]{3})/);if(x&&r.length<70&&!r.match(/\d{4,6}/)){g=x[1]+"-"+x[2];const d=r.match(a);d&&(f=d[1]);continue}if(g){const d=r.match(a);if(d&&!r.match(/\d{4,6}/)){f=d[1];continue}const c=r.match(/(\d{1,2})\s*(JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC).*?(\d{4,6})/i);if(c){const k=parseInt(c[3]);k>=1e3&&k<=99999&&t.push({sector:g,date:`2026-${n[c[2].toUpperCase()]}-${c[1].padStart(2,"0")}`,airline:d?d[1]:f,rate:k})}}}return t}function rt(e){const t=Z(e);if(!t.length){j();return}document.getElementById("prevBox").classList.add("on"),document.getElementById("prevCount").textContent=t.length+" entr"+(t.length===1?"y":"ies");const n=document.getElementById("prevBody");n.innerHTML=t.slice(0,60).map(a=>`
    <tr>
  ...
  `).join(""),t.length>60&&(n.innerHTML+=`<tr><td colspan="4" style="text-align:center;padding:10px;
      color:var(--z-text-soft);font-size:11px">+ ${t.length-60} more entries</td></tr>`)}function j(){document.getElementById("prevBox").classList.remove("on")}document.getElementById("resetBtn").addEventListener("click",()=>{D.value="",document.getElementById("charCount").textContent="0 characters",j(),R()});document.getElementById("submitBtn").addEventListener("click",async()=>{if(!T||!D.value.trim())return;const e=document.getElementById("submitBtn"),t=e.innerHTML;e.disabled=!0,e.innerHTML='<div class="spin"></div> Processing...';const n=document.getElementById("progBar"),a=document.getElementById("progFill");n.classList.add("on");let g=0;const f=setInterval(()=>{g=Math.min(g+Math.random()*13,85),a.style.width=g+"%"},280),b=Z(D.value),r={agent_id:T,raw_text:D.value.trim(),timestamp:new Date().toISOString(),source:"zamra-portal"},x={id:Date.now(),agent:T,time:new Date().toLocaleString("en-IN",{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"}),rows:b.length,status:"pen"};L.unshift(x),L.length>15&&L.pop(),P(),O();try{const d=await fetch(nt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)});if(clearInterval(f),a.style.width="100%",d.ok)x.status="ok",Y(b.length);else throw new Error(`Server error ${d.status}`)}catch(d){clearInterval(f),a.style.width="100%",d.message.startsWith("Server error")?(x.status="err",P(),O(),H("error","Submission Failed",d.message)):(x.status="ok",Y(b.length),H("warning","Webhook Not Connected","Data captured locally. Set the WEBHOOK constant to your n8n URL to go live."))}setTimeout(()=>{n.classList.remove("on"),a.style.width="0%",e.innerHTML=t,R()},900)});function Y(e){P(),O(),G+=e,U(),H("success","Submitted Successfully",`Agent ${T} — ${e} entries queued for processing.`),setTimeout(()=>{D.value="",document.getElementById("charCount").textContent="0 characters",j(),R()},500)}const lt={success:'<svg viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="7" stroke="currentColor" stroke-width="1.5"/><path d="M5 8l2 2 4-4" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>',error:'<svg viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="7" stroke="currentColor" stroke-width="1.5"/><path d="M5.5 5.5l5 5M10.5 5.5l-5 5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>',warning:'<svg viewBox="0 0 16 16" fill="none"><path d="M8 2L14 14H2L8 2z" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/><path d="M8 6.5v3M8 11v.5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>'};function H(e,t,n){const a=document.createElement("div");a.className=`toast ${e}`,a.innerHTML=`
    ...
    </button>`,document.getElementById("toastsEl").appendChild(a),setTimeout(()=>a.remove(),7e3)}function P(){localStorage.setItem("zt_hist",JSON.stringify(L))}function O(){const e=document.getElementById("historyWrap");if(!L.length){e.innerHTML=`
      <div class="h-empty">
  ...
  `).join("")}document.getElementById("clearBtn").addEventListener("click",()=>{L.length&&(L=[],G=0,P(),O(),U())});async function F(){const e=document.getElementById("origin").value,t=document.getElementById("destination").value,n=document.getElementById("flightList"),a=document.getElementById("loading"),g=document.getElementById("resultsHeader"),f=document.getElementById("origName"),b=document.getElementById("locName");n.innerHTML="",a.style.display="block",g.style.display="none";try{const r=await X(),x=`${e} ${t}`,d=r.find(i=>i.sectorCode===x);let c=[];if(d){const i=new Date;i.setHours(0,0,0,0);let A=await q({sectorId:d.id,startDate:i.toISOString()});A.sort((o,s)=>o.flightDate.getTime()===s.flightDate.getTime()?o.finalRate-s.finalRate:o.flightDate.getTime()-s.flightDate.getTime());const y=await W(),_={};y.forEach(o=>_[o.id]=o.name),c=A.map(o=>{const s={day:"2-digit",month:"short",year:"numeric"},m=o.flightDate.toLocaleDateString("en-GB",s).replace(/,/g,""),u=o.flightTime&&o.flightTime.split("-")[0]?o.flightTime.split("-")[0].trim():"TBA",B=o.flightTime&&o.flightTime.includes("-")?o.flightTime.split("-")[1].trim():"TBA";return{airline:_[o.airlineId]||"Unknown Airline",origin:e,destination:t,date:m,departure:u,arrival:B,price:"₹"+o.finalRate.toLocaleString("en-IN"),seats:o.seatsAvailable||0}})}if(a.style.display="none",g.style.display="block",f&&(f.innerText=e),b&&(b.innerText=t),!c||c.length===0){n.innerHTML=`<div class="text-center text-text-muted p-10 font-bold border-2 border-dashed border-border rounded-[24px] mt-6 bg-[#f8fafc]">No flights currently found from ${e} to ${t}. Try another destination.</div>`;return}let k="";c.forEach(i=>{let A="00",y="MTH";if(i.date){const B=i.date.split(" ");B.length>=2?(A=B[0],y=B[1]):(A=i.date,y="")}const o=`https://wa.me/919846606739?text=${encodeURIComponent(`Hello Zamra Travels, I'm interested in booking this flight:
...
Please confirm availability!`)}`;let s=(i.airline||"").toUpperCase().trim(),m="";const u={INDIGO:"/assets/img/flights/indigo.png","AIR INDIA EXPRESS":"/assets/img/flights/air-india-express.png","AIR ARABIA":"/assets/img/flights/air-arabia.png",FLYNAS:"/assets/img/flights/flynas.png","OMAN AIR":"/assets/img/flights/oman-air.png","SALAM AIR":"/assets/img/flights/salam-air.png","AIR INDIA":"/assets/img/flights/air-india.png",SAUDIA:"/assets/img/flights/saudia.png"};s.includes("EXPRESS")||s==="IX"?m=u["AIR INDIA EXPRESS"]:s.includes("INDIA")||s==="AI"?m=u["AIR INDIA"]:s.includes("SAUD")||s.includes("SOUD")||s==="SV"?m=u.SAUDIA:s.includes("INDIGO")||s==="6E"?m=u.INDIGO:s.includes("ARABIA")||s==="G9"?m=u["AIR ARABIA"]:s.includes("FLYNAS")||s==="XY"?m=u.FLYNAS:s.includes("OMAN")||s==="WY"?m=u["OMAN AIR"]:s.includes("SALAM")||s==="OV"?m=u["SALAM AIR"]:m=`https://flycreativekdr.com:8443/FlyCreativeNG/css2/img/Flight_Logo/${i.airline}.png`,k+=`
          ...
          <div class="flex flex-col gap-4 lg:hidden">
            ...
            <div class="flex flex-row items-center justify-between gap-2 px-2">
              <div class="text-left flex-none">
                <div class="text-[20px] font-heading font-bold text-navy leading-none tracking-tight mb-1">${i.origin}</div>
              ...
              <div class="text-right flex-none">
                <div class="text-[20px] font-heading font-bold text-navy leading-none tracking-tight mb-1">${i.destination}</div>
            ...
            <div class="flex sm:flex-row flex-col items-center justify-between w-full border-t border-border pt-4 gap-3 sm:gap-0">
              ...
              <a href="${o}" target="_blank" class="w-full sm:w-auto bg-gradient-to-r from-primary to-[#1558c0] text-white font-heading font-bold text-[14px] px-6 py-3 rounded-xl hover:shadow-[0_4px_14px_rgba(26,115,232,0.3)] hover:-translate-y-1 transition-all text-center whitespace-nowrap">
                Book Now
              </a>
            </div>
          </div>
          ...
          <div class="hidden lg:flex flex-row items-center justify-between gap-6">
            ...
            <div class="flex flex-1 flex-row items-center justify-between gap-8 px-6">
              ...
              <div class="flex items-center gap-6 lg:gap-8 mx-0">
                <div class="text-left w-[100px]">
                  ...
                  <div class="text-[22px] font-medium text-navy uppercase leading-none tracking-tight">${i.origin}</div>
                  <div class="text-[13px] font-medium text-text-muted mt-1 uppercase">${i.origin.substring(0,3)}</div>
                </div>
                ...
                <div class="text-left w-[100px]">
                  ...
                  <div class="text-[22px] font-medium text-navy uppercase leading-none tracking-tight">${i.destination}</div>
                  <div class="text-[13px] font-medium text-text-muted mt-1 uppercase">${i.destination.substring(0,3)}</div>
                </div>
              </div>
            ...
            <div class="flex flex-col items-center justify-center lg:w-[180px] shrink-0">
              <div class="bg-[#f8fafc] rounded-xl p-4 w-full flex flex-col items-center border border-border/50">
                ...
                <a href="${o}" target="_blank" class="w-full bg-gradient-to-r from-primary to-[#1558c0] text-white font-heading font-bold text-[15px] px-6 py-2.5 rounded-xl hover:shadow-[0_4px_14px_rgba(26,115,232,0.3)] hover:-translate-y-1 transition-all text-center whitespace-nowrap">
                  Book Now
                </a>
      ...
      `}),n.innerHTML=k}catch(r){a.style.display="none",H("error","Connection Error","Failed to fetch live flights. Please ensure the server is active."),console.error(r)}}window.searchFlights=F;
