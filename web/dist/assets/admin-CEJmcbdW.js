import"./index.esm-DPDniVF0.js";import{o as Ke,l as Ze}from"./auth-DLtQESii.js";import{a as Gt,d as ie,u as Be,c as Ce,e as Qe,f as ta,h as ea,i as aa,g as oe,j as na,k as sa,l as ia,m as oa,b as re,n as ra,o as da,p as la,q as ca,r as Ae,s as ma,t as ua,v as pa,w as ga,x as fa,y as ba,z as ha,A as ya,B as va,C as xa,D as wa,E as $a,F as Ea,G as Ia,H as Sa,I as Ba,J as Ca,K as Aa,L as La,M as Ta,N as ka,O as Da,P as Ma,Q as Fa}from"./db-Dzh3zJx8.js";import"./firebase-config-CsZGR70X.js";async function ve(t,e,a,s,i){const n=`Generating ${t} Video... Please remain on this tab.`;return window.toast&&window.toast("info","Video Generation",n),new Promise(async(l,r)=>{try{let at=function(y,A,L,U,N){c.beginPath(),c.moveTo(y+N,A),c.lineTo(y+L-N,A),c.arcTo(y+L,A,y+L,A+N,N),c.lineTo(y+L,A+U-N),c.arcTo(y+L,A+U,y+L-N,A+U,N),c.lineTo(y+N,A+U),c.arcTo(y,A+U,y,A+U-N,N),c.lineTo(y,A+N),c.arcTo(y,A,y+N,A,N),c.closePath()},st=function(y){const A=y-$t;if(A>vt){try{Q.stop()}catch(D){console.error("Error stopping recorder",D)}return}c.fillStyle="#f8fafc",c.fillRect(0,0,o,d);const L=t==="9x16"?400:300;if(c.fillStyle="#1e293b",c.fillRect(0,0,o,L),B.complete&&B.width>0){c.globalAlpha=.2;const D=Math.max(o/B.width,L/B.height),j=B.width*D,Z=B.height*D,Pt=(o-j)/2,It=(L-Z)/2;c.drawImage(B,Pt,It,j,Z),c.globalAlpha=1}const U=c.createLinearGradient(0,0,0,L);U.addColorStop(0,"#1e293b"),U.addColorStop(1,"transparent"),c.fillStyle=U,c.globalAlpha=.8,c.fillRect(0,0,o,L),c.globalAlpha=1,c.textAlign="center",c.textBaseline="middle";const N=c.createLinearGradient(0,0,o,0);N.addColorStop(0,"#2563eb"),N.addColorStop(.5,"#60a5fa"),N.addColorStop(1,"#1558c0"),c.fillStyle=N,c.fillRect(0,0,o,16);const Et=200,it=40,yt=60;c.fillStyle="rgba(37, 99, 235, 0.4)",at(o/2-Et/2,yt,Et,it,20),c.fill(),c.strokeStyle="rgba(37, 99, 235, 0.6)",c.lineWidth=1,c.stroke(),c.fillStyle="#bfdbfe",c.font="bold 16px Arial, sans-serif",c.fillText("EXCLUSIVE DEALS",o/2,yt+it/2),c.fillStyle="#ffffff",c.font="900 "+(t==="16x9"?"70px":"56px")+" Arial, sans-serif",c.fillText(x,o/2,yt+80),c.fillStyle="#dbeafe",c.font="700 24px Arial, sans-serif",c.fillText("SPECIAL FARES AVAILABLE NOW",o/2,yt+140);const ct=L+60,tt=90,G=t==="9x16"?40:t==="1x1"?80:160,C=o-G*2;c.fillStyle="#64748b",c.font="bold 18px Arial, sans-serif",c.textAlign="left",c.fillText("DATE",G+20,ct-20),c.textAlign="center",c.fillText("SECTOR",G+C*.25,ct-20),c.fillText("AIRLINE",G+C*.45,ct-20),c.fillText("TIME",G+C*.65,ct-20),c.textAlign="right",c.fillText("FARE",G+C-20,ct-20);for(let D=0;D<E.length;D++){const j=E[D],Z=1e3+D*800;if(A<Z)continue;const It=Math.min(1,(A-Z)/500),Xt=20*(1-It),ot=ct+D*tt+Xt;c.globalAlpha=It,D%2===0&&(c.fillStyle="#ffffff",at(G,ot,C,tt-10,12),c.fill()),c.fillStyle="#0f172a",c.textBaseline="middle";const ue=j.flightDate instanceof Date?j.flightDate.toLocaleDateString("en-GB",{day:"2-digit",month:"short"}).toUpperCase():j.flightDate;c.textAlign="left",c.font="900 26px Arial, sans-serif",c.fillText(ue,G+20,ot+tt/2-5),c.font="700 22px Arial, sans-serif",c.fillStyle="#2563eb",c.textAlign="center";const Ye=g[j.sectorId]||j.sectorId;c.fillText(Ye,G+C*.25,ot+tt/2-5),c.fillStyle="#0f172a";const pe=G+C*.45,At=v(j.airlineId),Ut=At?P[At.id]:null;if(Ut&&Ut.width>0){const xt=Math.min(100,Ut.width),ye=40;c.drawImage(Ut,pe-xt/2,ot+tt/2-5-ye/2,xt,ye)}else{c.font="700 20px Arial, sans-serif",c.textAlign="center";const xt=(At==null?void 0:At.name)||j.airlineId||"—";c.fillText(xt,pe,ot+tt/2-5)}let qt=j.flightTime||"—";if(qt.includes("-")){const xt=qt.split("-");qt=`${xt[0].trim()} - ${xt[1].trim()}`}c.font="800 22px Arial, sans-serif",c.textAlign="center",c.fillText(qt,G+C*.65,ot+tt/2-5);const ge=`₹${(j.finalRate||0).toLocaleString()}`;c.font="900 26px Arial, sans-serif",c.textAlign="right";const Xe=c.measureText(ge).width,fe=G+C-20,be=Xe+40,he=50;c.fillStyle="#0f172a",at(fe-be,ot+tt/2-5-he/2,be,he,12),c.fill(),c.fillStyle="#ffffff",c.fillText(ge,fe-20,ot+tt/2-5),c.globalAlpha=1}const q=1e3+E.length*800+500;if(A>q){const D=Math.min(1,(A-q)/500);c.globalAlpha=D;const j=100,Z=d-j+20*(1-D);c.fillStyle="#ffffff",c.fillRect(0,d-j,o,j),c.fillRect(0,Z,o,j),c.fillStyle="#f1f5f9",c.fillRect(0,d-j,o,2),T.complete&&T.width>0&&c.drawImage(T,G,d-j/2-24,48,48),c.fillStyle="#1e293b",c.font="900 24px Arial, sans-serif",c.textAlign="left",c.textBaseline="middle",c.fillText("Zamra Travels",G+64,d-j/2),c.font="700 20px Arial, sans-serif",c.textAlign="right",c.fillText("zamratravels.com  |  +91 98466 06739",o-G,d-j/2),c.globalAlpha=1}requestAnimationFrame(st)},o,d;if(t==="1x1")o=1080,d=1080;else if(t==="9x16")o=1080,d=1920;else if(t==="16x9")o=1920,d=1080;else throw new Error("Invalid ratio selected");const m=document.createElement("canvas");m.width=o,m.height=d;const c=m.getContext("2d");c.imageSmoothingEnabled=!0;let x="MULTIPLE → SECTORS";if(a!=="all"){const y=s.find(U=>U.id===a),A=y?(y.sectorFrom||"DEP").toUpperCase():"DEP",L=y?(y.sectorTo||"ARR").toUpperCase():"ARR";x=`${A} → ${L}`}const u=new Map;e.forEach(y=>{const A=y.flightDate instanceof Date?y.flightDate.getTime():y.flightDate,L=`${y.sectorId}_${y.airlineId}_${A}_${y.flightTime}`;u.has(L)?y.finalRate<u.get(L).finalRate&&u.set(L,y):u.set(L,y)});const E=Array.from(u.values()).sort((y,A)=>{let L=y.flightDate,U=A.flightDate;return L instanceof Date&&(L=L.getTime()),U instanceof Date&&(U=U.getTime()),L-U}),p={};i.forEach(y=>{y.id&&(p[y.id.trim().toLowerCase()]=y),y.code&&(p[y.code.trim().toLowerCase()]=y),y.name&&(p[y.name.trim().toLowerCase()]=y)});const v=y=>y?p[String(y).trim().toLowerCase()]:null,g={};s.forEach(y=>{g[y.id]=y.sectorCode||y.id});const f=y=>String(y||"").trim().replace(/[^a-z0-9]+/gi,"-").replace(/^-+|-+$/g,"").toLowerCase();let b="all-sectors";if(a!=="all"){const y=s.find(L=>L.id===a),A=(y==null?void 0:y.sectorCode)||(y?`${y.sectorFrom||""}-${y.sectorTo||""}`:"")||g[a]||a;b=f(A)||f(a)||"sector"}async function w(y){if(!y)return null;try{const A=await fetch(y);if(!A.ok)return null;const L=await A.blob(),U=URL.createObjectURL(L);return new Promise((N,Et)=>{const it=new Image;it.onload=()=>N(it),it.onerror=()=>N(null),it.src=U})}catch{return null}}const B=new Image;await new Promise(y=>{B.onload=y,B.onerror=y,B.src="/assets/img/hero-bg.webp"});const T=new Image;await new Promise(y=>{T.onload=y,T.onerror=y,T.src="/assets/img/logo.webp"});const P={},_=[...new Set(E.map(y=>y.airlineId))].map(y=>v(y)).filter(y=>y&&y.logoUrl);await Promise.all(_.map(async y=>{const A=await w(y.logoUrl);A&&(P[y.id]=A)}));const k=m.captureStream(30);let H="video/mp4";MediaRecorder.isTypeSupported(H)||(H="video/webm; codecs=h264",MediaRecorder.isTypeSupported(H)||(H="video/webm"));const Q=new MediaRecorder(k,{mimeType:H}),V=[];Q.ondataavailable=y=>{y.data&&y.data.size>0&&V.push(y.data)},Q.start(100);const vt=1e4+E.length*1500,$t=performance.now();requestAnimationFrame(st),Q.onstop=()=>{const y=new Blob(V,{type:H}),A=URL.createObjectURL(y),L=document.createElement("a");L.href=A,L.download=`zamra-video-${t}-${b}-${Date.now()}.mp4`,L.style.display="none",document.body.appendChild(L),L.click(),setTimeout(()=>{document.body.removeChild(L),URL.revokeObjectURL(A)},100),window.toast&&window.toast("success","Video Generated",`Your ${t} video has been downloaded!`),l()},Q.onerror=y=>{console.error("Recorder Error:",y),window.toast&&window.toast("error","Generation Error","Failed to encode the video stream."),r(y)}}catch(o){console.error(o),window.toast&&window.toast("error","Generation Failed",o.message),r(o)}})}let Y=[],M=[],O=[],_t=[],Zt=[],Qt=[],te=[],ee=[],ae=[],W=[],et=[],X={},nt=new Set,mt=new Set;function St(t){return t==null?t:String(t).replace(/damamm/gi,e=>e===e.toUpperCase()?"DAMMAM":e===e.toLowerCase()?"dammam":"Dammam")}function Le(t={}){return{...t,sectorFrom:St(t.sectorFrom||""),sectorTo:St(t.sectorTo||""),sectorCode:St(t.sectorCode||"")}}function de(t=[]){return t.map(e=>Le(e))}function S(t=""){return String(t).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function R(t,e=0){const a=Number(t);return Number.isFinite(a)?a:e}const Te=[5,7,10],Vt=[20,25,30,35,40];function Bt(t=[],e=0){const a=Math.max(0,ht(e)),s=[...new Set(t.map(n=>Math.max(0,ht(n))))].filter(n=>n>0).sort((n,l)=>n-l);if(!s.length)return"";const i=s.includes(a)?a:s[0];return s.map(n=>`<option value="${n}" ${n===i?"selected":""}>${n} Kg</option>`).join("")}function ht(t){if(t==null||t==="")return 0;const e=parseFloat(String(t).replace(/[^\d.]/g,""));return Number.isFinite(e)?e:0}function xe(t,e="—"){if(t==null||t==="")return e;const a=String(t).trim();return a?/^\d+(\.\d+)?(\s*kg)?$/i.test(a)?`${ht(a)} Kg`:a.toUpperCase():e}function Mt(t){if(!t)return null;if(t instanceof Date)return Number.isNaN(t.getTime())?null:t;const e=new Date(t);return Number.isNaN(e.getTime())?null:e}function Ft(t){const e=Mt(t);if(!e)return"";const a=e.getTimezoneOffset();return new Date(e.getTime()-a*60*1e3).toISOString().split("T")[0]}function ke(t){if(!t)return null;const e=new Date(`${t}T00:00:00`);return Number.isNaN(e.getTime())?null:e}function ja(t){if(!t)return null;const e=new Date(`${t}T00:00:00`);return Number.isNaN(e.getTime())?null:e.getTime()}function Ra(t){if(!t)return null;const e=new Date(`${t}T23:59:59.999`);return Number.isNaN(e.getTime())?null:e.getTime()}let rt={agents:{key:"id",asc:!0},sectors:{key:"id",asc:!0},airlines:{key:"name",asc:!0},visas:{key:"countryName",asc:!0},visaStampings:{key:"country",asc:!0},attestations:{key:"country",asc:!0},passportServices:{key:"type",asc:!0},tours:{key:"title",asc:!0},hajjUmrah:{key:"title",asc:!0},reportFares:{key:"flightDate",asc:!0},databaseFares:{key:"flightDate",asc:!0}},Wt={agents:"",sectors:"",airlines:"",visas:"",visaStampings:"",attestations:"",passportServices:"",tours:"",hajjUmrah:""},I={agents:1,sectors:1,airlines:1,visas:1,visaStampings:1,attestations:1,passportServices:1,tours:1,hajjUmrah:1,reportFares:1,databaseFares:1},J={agents:10,sectors:25,airlines:10,visas:10,visaStampings:10,attestations:10,passportServices:10,tours:10,hajjUmrah:10,reportFares:10,databaseFares:25};const F={search:"",agentId:"all",sectorId:"all",airlineId:"all",status:"all",startDate:"",endDate:""};function ut(t,e){var l;let a=t;const s=(l=Wt[e])==null?void 0:l.toLowerCase();s&&e==="agents"?a=a.filter(r=>(r.name||"").toLowerCase().includes(s)||(r.email||"").toLowerCase().includes(s)||(r.contactPhone||"").toLowerCase().includes(s)||(r.id||"").toLowerCase().includes(s)):s&&e==="sectors"?a=a.filter(r=>(r.sectorFrom||"").toLowerCase().includes(s)||(r.sectorTo||"").toLowerCase().includes(s)||(r.sectorCode||"").toLowerCase().includes(s)):s&&e==="airlines"?a=a.filter(r=>(r.name||"").toLowerCase().includes(s)||(r.code||"").toLowerCase().includes(s)):s&&e==="visas"?a=a.filter(r=>(r.countryName||"").toLowerCase().includes(s)||(r.visaType||"").toLowerCase().includes(s)):s&&e==="visaStampings"?a=a.filter(r=>(r.country||"").toLowerCase().includes(s)||(r.description||"").toLowerCase().includes(s)):s&&e==="attestations"?a=a.filter(r=>(r.country||"").toLowerCase().includes(s)||(r.certificate||"").toLowerCase().includes(s)):s&&e==="passportServices"?a=a.filter(r=>(r.type||"").toLowerCase().includes(s)||(r.description||"").toLowerCase().includes(s)):s&&e==="tours"?a=a.filter(r=>(r.title||"").toLowerCase().includes(s)||(r.category||"").toLowerCase().includes(s)||(r.duration||"").toLowerCase().includes(s)):s&&e==="hajjUmrah"&&(a=a.filter(r=>(r.title||"").toLowerCase().includes(s)||(r.type||"").toLowerCase().includes(s)||(r.departureCity||"").toLowerCase().includes(s)||(r.airline||"").toLowerCase().includes(s)));const{key:i,asc:n}=rt[e];return i&&(a=[...a].sort((r,o)=>{let d=r[i],m=o[i];if(d instanceof Date&&(d=d.getTime()),m instanceof Date&&(m=m.getTime()),i==="id"){const c=parseInt(d),x=parseInt(m);if(!isNaN(c)&&!isNaN(x))return n?c-x:x-c}return typeof d=="string"&&(d=d.toLowerCase()),typeof m=="string"&&(m=m.toLowerCase()),d<m?n?-1:1:d>m?n?1:-1:0})),a}function Ht(t){document.querySelectorAll(`th[data-sort-tab="${t}"] i`).forEach(a=>{a.className="bi bi-arrow-down-up opacity-30 group-hover:opacity-100 transition-opacity ml-1 text-[11px]"});const e=document.querySelector(`th[data-sort-tab="${t}"][data-sort-key="${rt[t].key}"]`);if(e){const a=e.querySelector("i");a&&(a.className=`bi bi-arrow-${rt[t].asc?"up":"down"} opacity-100 ml-1 text-[11px] text-primary`)}}document.addEventListener("click",t=>{const e=t.target.closest("th[data-sort-tab]");if(!e)return;const a=e.dataset.sortTab,s=e.dataset.sortKey;rt[a].key===s?rt[a].asc=!rt[a].asc:(rt[a].key=s,rt[a].asc=!0),a==="agents"?pt(!1):a==="sectors"?gt(!1):a==="airlines"?wt(!1):a==="visas"?lt(!1):a==="tours"?Jt(!1):a==="hajjUmrah"?Yt(!1):a==="reportFares"&&W.length?jt(W):a==="databaseFares"&&z()});document.documentElement.style.visibility="hidden";Ke(async t=>{if(!t){window.location.href="/login.html";return}document.documentElement.style.visibility="visible";const e=document.getElementById("admin-user-name");e&&(e.textContent=t.email.split("@")[0]),await Na(),qe(),await De()});document.addEventListener("DOMContentLoaded",()=>{const t=document.getElementById("admin-logout-btn");t&&t.addEventListener("click",async()=>{(await Ze()).success&&(window.location.href="/login.html")}),Pa(),Ha(),un()});async function Na(){try{const[t,e,a,s]=await Promise.all([Ce(),oe(),re(),Ae()]);Y=t,M=de(e),O=a,_t=s}catch(t){console.error("loadGlobalData error:",t)}}function Ha(){var i;const t=document.querySelectorAll(".nav-link"),e=document.querySelectorAll(".tab-content"),a=document.getElementById("page-title"),s=document.getElementById("admin-tab-select");if(t.forEach(n=>{n.addEventListener("click",async l=>{var d;l.preventDefault(),t.forEach(m=>{m.classList.remove("active","text-primary"),m.classList.add("text-text-muted")}),n.classList.remove("text-text-muted"),n.classList.add("active","text-primary");const r=n.getAttribute("data-tab"),o=n.getAttribute("data-title");e.forEach(m=>m.classList.remove("active")),(d=document.getElementById(r))==null||d.classList.add("active"),a&&o&&(a.textContent=o),s&&r&&(s.value=r),await De()})}),s){const n=document.querySelector(".nav-link.active");(i=n==null?void 0:n.dataset)!=null&&i.tab&&(s.value=n.dataset.tab),s.addEventListener("change",()=>{const l=s.value,r=document.querySelector(`.nav-link[data-tab="${l}"]`);r&&r.dispatchEvent(new MouseEvent("click",{bubbles:!0,cancelable:!0}))})}}async function De(){const t=document.querySelector(".tab-content.active");if(!t)return;const e=t.id;e==="agents-tab"?await pt():e==="sectors-tab"?await gt():e==="flights-tab"?await wt():e==="dashboard-tab"?await Ua():e==="reports-tab"?await Ja():e==="database-tab"?await ce():e==="visas-tab"?await lt():e==="tours-tab"?await Jt():e==="hajjumrah-tab"?await Yt():e==="agent-sheets-tab"?(qe(),Nt(),bt()):e==="eticket-tab"&&await vn()}function Pa(){const t=document.getElementById("admin-modal"),e=document.getElementById("modal-close-btn");e&&e.addEventListener("click",()=>t.close()),t==null||t.addEventListener("click",a=>{a.target===t&&t.close()})}function dt(t,e,a=!1){const s=document.getElementById("admin-modal");document.getElementById("modal-title").textContent=t,s.classList.toggle("max-w-lg",!a),s.classList.toggle("max-w-2xl",a);const i=document.getElementById("modal-body");i.innerHTML=e,s.showModal()}async function Ua(){var s,i,n,l,r;if(!document.getElementById("dashboard-tab"))return;const e=document.getElementById("poster-sector-sel");e&&e.options.length<=2&&M.forEach(o=>{const d=new Option(o.sectorCode,o.id);e.appendChild(d)});const a=document.getElementById("poster-generate-btn");a&&!a.dataset.wired&&(a.dataset.wired="1",a.addEventListener("click",async()=>{const o=document.getElementById("poster-start-date"),d=document.getElementById("poster-end-date"),m=e==null?void 0:e.value,c=(o==null?void 0:o.value)||null,x=(d==null?void 0:d.value)||null;if(!m){$("warning","Validation Error","Please select a sector to generate the poster.");return}a.disabled=!0,a.textContent="Generating…";try{const u=await Gt({sectorId:m,startDate:c,endDate:x,includeHidden:!1});if(!u||!u.length){$("warning","No Fares","No live fares found for the selected sector and dates."),document.getElementById("poster-preview-container").classList.add("hidden");return}await qa(u,m)}catch(u){$("error","Generation Failed",u.message)}finally{a.disabled=!1,a.textContent="Generate Poster"}}),(s=document.getElementById("poster-download-jpg"))==null||s.addEventListener("click",()=>we("jpeg")),(i=document.getElementById("poster-download-pdf"))==null||i.addEventListener("click",()=>we("pdf")),(n=document.getElementById("poster-download-vid-1x1"))==null||n.addEventListener("click",()=>Kt("1x1")),(l=document.getElementById("poster-download-vid-9x16"))==null||l.addEventListener("click",()=>Kt("9x16")),(r=document.getElementById("poster-download-vid-16x9"))==null||r.addEventListener("click",()=>Kt("16x9")))}async function Kt(t){const e=document.getElementById("poster-sector-sel"),a=document.getElementById("poster-start-date"),s=document.getElementById("poster-end-date"),i=e==null?void 0:e.value,n=(a==null?void 0:a.value)||null,l=(s==null?void 0:s.value)||null;if(!i){$("warning","Validation Error","Please select a sector to generate the poster.");return}try{const r=await Gt({sectorId:i,startDate:n,endDate:l,includeHidden:!1});if(!r||!r.length){$("warning","No Fares","No live fares found for the selected sector and dates.");return}if(i==="all"){const o=new Map;r.forEach(u=>{const h=u.sectorId||"unknown";o.has(h)||o.set(h,[]),o.get(h).push(u)});const d=new Map(M.map((u,h)=>[u.id,h])),m=Array.from(o.keys()).sort((u,h)=>{const E=d.get(u)??1e9,p=d.get(h)??1e9;return E!==p?E-p:String(u).localeCompare(String(h))});$("info","Video Generation",`Generating ${m.length} videos. This may take a while…`);let c=0,x=0;for(const u of m){const h=o.get(u)||[];if(h.length)try{await ve(t,h,u,M,O),c+=1}catch(E){x+=1,console.error("Video generation failed for sector",u,E)}}c&&$("success","Video Generation",`Downloaded ${c} videos successfully.`),x&&$("error","Video Generation",`${x} videos failed to generate. Check console for details.`);return}await ve(t,r,i,M,O)}catch(r){console.error("Video generation failed",r),$("error","Generation Failed",r.message||"Video generation failed.")}}async function qa(t,e){const a=document.getElementById("poster-preview-container"),s=document.getElementById("poster-render-stack"),i=document.querySelector('[data-poster-template="1"]')||document.getElementById("poster-render-frame");if(!a||!s||!i)return;s.querySelectorAll('[data-poster-clone="1"]').forEach(g=>g.remove());const n=new Map;t.forEach(g=>{const f=g.flightDate instanceof Date?g.flightDate.getTime():g.flightDate,b=`${g.sectorId}_${g.airlineId}_${f}_${g.flightTime}`;n.has(b)?g.finalRate<n.get(b).finalRate&&n.set(b,g):n.set(b,g)});const r=Array.from(n.values()).sort((g,f)=>{let b=g.flightDate,w=f.flightDate;return b instanceof Date&&(b=b.getTime()),w instanceof Date&&(w=w.getTime()),b-w}),o={};O.forEach(g=>{g.id&&(o[g.id.trim().toLowerCase()]=g),g.code&&(o[g.code.trim().toLowerCase()]=g),g.name&&(o[g.name.trim().toLowerCase()]=g)});const d=g=>g?o[String(g).trim().toLowerCase()]:null;async function m(g){try{const f=await fetch(g);if(!f.ok)return null;const b=await f.blob();return URL.createObjectURL(b)}catch{return null}}const c=[...new Set(r.map(g=>g.airlineId))].map(g=>d(g)).filter(g=>g&&g.logoUrl),x={};await Promise.all(c.map(async g=>{const f=await m(g.logoUrl);f&&(x[g.id]=f)}));const u={};M.forEach(g=>u[g.id]=g.sectorCode);const h=g=>String(g||"").trim().replace(/[^a-z0-9]+/gi,"-").replace(/^-+|-+$/g,"").toLowerCase(),E=(g,f,b)=>{const w=g.querySelector("[data-poster-title]")||g.querySelector("#poster-sector-title"),B=g.querySelector("[data-poster-tbody]")||g.querySelector("#poster-fares-tbody");if(!w||!B)return;const T=M.find(k=>k.id===b);let P=T?(T.sectorFrom||"DEP").toUpperCase():"DEP",_=T?(T.sectorTo||"ARR").toUpperCase():"ARR";if(!T){const k=u[b]||b,H=String(k).match(/^\s*([A-Za-z0-9]+)\s*[-→>]\s*([A-Za-z0-9]+)\s*$/);H?(P=H[1].toUpperCase(),_=H[2].toUpperCase()):(P=String(k).toUpperCase(),_="")}w.innerHTML=_?`${P} <span style="color:#60a5fa;font-weight:900;">&#8594;</span> ${_}`:`${P}`,B.innerHTML=f.map((k,H)=>{const Q=k.flightDate instanceof Date?k.flightDate.toLocaleDateString("en-GB",{day:"2-digit",month:"short"}).toUpperCase():k.flightDate,V=d(k.airlineId),vt=H%2===0?"#ffffff":"#f8fafc",$t=V?x[V.id]:null,at=$t?`<img src="${$t}" style="height:24px;max-width:80px;object-fit:contain;display:block;margin:0 auto;" alt="${(V==null?void 0:V.name)||""}">`:`<span style="font-weight:700;color:#0f172a;display:block;text-align:center;font-size:13px;white-space:nowrap;">${(V==null?void 0:V.name)||k.airlineId||"—"}</span>`,st=`<span style="font-weight:700;color:#2563eb;background-color:rgba(37,99,235,0.1);padding:4px 8px;border-radius:6px;font-size:12px;text-align:center;white-space:nowrap;">${u[k.sectorId]||k.sectorId}</span>`;let y='<span style="color:#94a3b8;font-size:13px;">—</span>';if(k.flightTime){const A=k.flightTime.split("-").map(L=>L.trim());A.length>=2?y=`<span style="font-weight:700;font-size:13px;color:#0f172a;white-space:nowrap;">${A[0]} - ${A[1]}</span>`:y=`<span style="font-weight:700;font-size:13px;color:#0f172a;white-space:nowrap;">${k.flightTime}</span>`}return`
        ...
        </tr>`}).join("")},p=new Map;r.forEach(g=>{const f=g.sectorId||"unknown";p.has(f)||p.set(f,[]),p.get(f).push(g)});let v=[];if(e==="all"){v=Array.from(p.keys());const g=new Map(M.map((f,b)=>[f.id,b]));v.sort((f,b)=>{const w=g.get(f)??1e9,B=g.get(b)??1e9;return w!==B?w-B:String(f).localeCompare(String(b))})}else v=[e];v.forEach((g,f)=>{const b=p.get(g)||[];if(!b.length)return;let w=i;if(f>0){w=i.cloneNode(!0),w.dataset.posterClone="1",w.removeAttribute("data-poster-template"),w.querySelectorAll("#poster-sector-title, #poster-fares-tbody").forEach(P=>P.removeAttribute("id"));const B=u[g]||g,T=h(B)||`sector-${f+1}`;w.id=`poster-render-frame-${T}-${f+1}`,s.appendChild(w)}else w.id="poster-render-frame";w.dataset.posterFrame="1",w.dataset.sectorId=g,w.dataset.sectorCode=u[g]||g,E(w,b,g)}),a.classList.remove("hidden"),a.classList.add("flex")}function Me(t){if(!t||t.nodeType!==1)return;const e=window.getComputedStyle(t),a=["color","backgroundColor","borderTopColor","borderBottomColor","borderLeftColor","borderRightColor","outlineColor"];for(const s of a){const i=e.getPropertyValue(s);if(i&&!i.startsWith("rgb")&&!i.startsWith("#")&&i!=="transparent"&&i!=="initial")try{t.style[s]=i}catch{}}for(const s of t.children)Me(s)}async function we(t){const e=document.getElementById("poster-render-stack"),a=e?Array.from(e.querySelectorAll('[data-poster-frame="1"]')):[];if(!a.length)return;const s=document.getElementById("poster-download-jpg"),i=document.getElementById("poster-download-pdf");s&&(s.disabled=!0),i&&(i.disabled=!0);const n=m=>String(m||"").trim().replace(/[^a-z0-9]+/gi,"-").replace(/^-+|-+$/g,"").toLowerCase(),l=Date.now(),r=a.length>1;$("info","Generating Export",r?`Rendering ${a.length} posters. Your browser may ask to allow multiple downloads…`:"Please wait while we render your poster…");let o=0,d=null;for(let m=0;m<a.length;m++){const c=a[m],x=c.style.transform;c.style.transform="none";try{await Promise.all(Array.from(c.querySelectorAll("img")).map(g=>g.complete?Promise.resolve():new Promise(f=>{g.onload=f,g.onerror=f})));const u=await html2canvas(c,{scale:2,useCORS:!1,allowTaint:!0,backgroundColor:"#ffffff",logging:!1,onclone:g=>{const f=c.id?g.getElementById(c.id):null;f&&Me(f)}}),h=u.toDataURL("image/jpeg",.95),E=c.dataset.sectorCode||c.dataset.sectorId||`poster-${m+1}`,v=`zamra-poster-${n(E)||`poster-${m+1}`}-${l}`;if(t==="jpeg"){const g=document.createElement("a");g.download=`${v}.jpg`,g.href=h,g.click()}else if(t==="pdf"){const g=window.jspdf&&window.jspdf.jsPDF||window.jsPDF||window.jspdf;if(!g)throw new Error("jsPDF library not loaded.");const f=96/25.4,b=u.width/2/f,w=u.height/2/f,B=new g({orientation:b>w?"landscape":"portrait",unit:"mm",format:[b,w]});B.addImage(h,"JPEG",0,0,b,w),B.save(`${v}.pdf`)}o+=1}catch(u){console.error("Poster export error:",u),d||(d=u)}finally{c.style.transform=x}}if(o){const m=r?`Downloaded ${o} ${t==="pdf"?"PDFs":"JPEGs"} successfully.`:`${t==="pdf"?"PDF":"JPEG"} poster saved successfully.`;$("success","Downloaded!",m)}d&&$("error","Export Failed",d.message||"There was an error generating the export."),s&&(s.disabled=!1),i&&(i.disabled=!1)}function jt(t){const e=document.getElementById("report-fares-results");if(!e)return;if(!t||!t.length){e.innerHTML=`<div class="admin-empty-state">
    ...
    </div>`;return}const a=Object.fromEntries(Y.map(u=>[u.id,u.name])),s=Object.fromEntries(M.map(u=>[u.id,u.sectorCode])),i=Object.fromEntries(O.map(u=>[u.id,u.code])),{key:n,asc:l}=rt.reportFares,r=[...t].sort((u,h)=>{let E=u[n],p=h[n];return E instanceof Date&&(E=E.getTime()),p instanceof Date&&(p=p.getTime()),typeof E=="string"&&(E=E.toLowerCase()),typeof p=="string"&&(p=p.toLowerCase()),E<p?l?-1:1:E>p?l?1:-1:0}),o=J.reportFares,d=Math.max(1,Math.ceil(t.length/o));I.reportFares>d&&(I.reportFares=d);const m=(I.reportFares-1)*o,c=r.slice(m,m+o),x=(u,h)=>`<th class="cursor-pointer group whitespace-nowrap" data-sort-tab="reportFares" data-sort-key="${u}">${h} <i class="bi bi-arrow-down-up opacity-30 group-hover:opacity-100 transition-opacity ml-1 text-[11px]"></i></th>`;e.innerHTML=`
    ...
    <div id="reportFares-pagination-footer" class="border-t border-slate-100 bg-slate-50/80 rounded-b-2xl"></div>`,Ct("reportFares",t.length,d,m,o),window.__deleteFare=async u=>{if(confirm("Delete this fare?"))try{await ie(u),W=W.filter(h=>h.id!==u),$("success","Deleted","Fare removed."),jt(W)}catch(h){$("error","Error",h.message)}},window.__toggleFare=async(u,h)=>{try{await Be(u,{isHidden:h}),W=W.map(E=>E.id===u?{...E,isHidden:h}:E),$("success","Updated",`Fare ${h?"hidden":"shown"}.`),jt(W)}catch(E){$("error","Error",E.message)}},Ht("reportFares")}async function pt(t=!0){t&&(Y=await Ce(),I.agents=1);const e=document.querySelector("#agents-tab .admin-table tbody");if(!e)return;const a=document.getElementById("agents-search"),s=document.getElementById("agents-limit");a&&!a.dataset.wired&&(a.dataset.wired="1",s&&(s.dataset.wired="1"),a.addEventListener("input",m=>{Wt.agents=m.target.value,I.agents=1,pt(!1)}),s&&s.addEventListener("change",m=>{J.agents=parseInt(m.target.value),I.agents=1,pt(!1)}));const i=ut(Y,"agents"),n=J.agents,l=Math.max(1,Math.ceil(i.length/n));I.agents>l&&(I.agents=l);const r=(I.agents-1)*n,o=i.slice(r,r+n);e.innerHTML=o.length?o.map(m=>Oa(m)).join(""):'<tr><td colspan="7" class="text-center py-8 text-text-muted">No agents yet. Click "+ Add Agent" to get started.</td></tr>',Ct("agents",i.length,l,r,n),_a();const d=document.getElementById("agents-add-btn");d&&!d.dataset.wired&&(d.dataset.wired="1",d.addEventListener("click",()=>Fe(null))),Ht("agents")}function Oa(t){const e=t.isActive!==!1?'<span class="admin-status-pill admin-status-active">Active</span>':'<span class="admin-status-pill admin-status-inactive">Hidden</span>',a=t.commission!==void 0?`₹${Number(t.commission).toLocaleString()}`:"—";return`<tr data-agent-id="${t.id}">
  ...
  </tr>`}function _a(){const t=document.querySelector("#agents-tab .admin-table tbody");!t||t.dataset.actionsWired||(t.dataset.actionsWired="1",t.addEventListener("click",async e=>{const a=e.target.closest("[data-action]");if(!a)return;const s=a.dataset.action,i=a.dataset.id,n=Y.find(l=>l.id===i);if(s==="edit-agent"&&Fe(n),s==="delete-agent"){if(!confirm(`Delete agent "${n==null?void 0:n.name}"? This does NOT delete their fares.`))return;try{await Qe(i),$("success","Deleted",`Agent "${n==null?void 0:n.name}" removed.`),await pt()}catch(l){$("error","Error",l.message)}}if(s==="toggle-agent"){const r=!(a.dataset.active==="true");a.disabled=!0,a.textContent="Working…";try{const o=await ta(i,r);$("success",r?"Agent Shown":"Agent Hidden",o.message),await pt()}catch(o){$("error","Toggle Failed",o.message),await pt()}}}))}function Ct(t,e,a,s,i){const n=document.getElementById(`${t}-pagination-footer`);if(!n)return;const l=Math.min(s+i,e),r=I[t];n.innerHTML=`
    <div class="flex items-center justify-between px-2 py-3 text-sm text-text-muted overflow-x-auto whitespace-nowrap">
      <span>Showing ${e?s+1:0} to ${l} of ${e} entries</span>
      <div class="admin-pagination-wrap">
        <button data-pg-action="prev" class="admin-pagination-btn" ${r<=1?"disabled":""}>Previous</button>
        ${function(){return a<=7?Array.from({length:a},(o,d)=>d+1):r<=4?[1,2,3,4,5,"...",a]:r>=a-3?[1,"...",a-4,a-3,a-2,a-1,a]:[1,"...",r-1,r,r+1,"...",a]}().map(o=>o==="..."?'<span class="admin-pagination-btn" style="cursor:default;opacity:0.5;background:transparent;">...</span>':`<button data-pg-action="goto" data-pg="${o}" class="admin-pagination-btn ${o===r?"admin-pagination-btn-active":""}">${o}</button>`).join("")}
        <button data-pg-action="next" class="admin-pagination-btn" ${r>=a?"disabled":""}>Next</button>
      </div>
    </div>`,n.dataset.wired||(n.dataset.wired="1",n.addEventListener("click",o=>{const d=o.target.closest("[data-pg-action]");if(!d||d.disabled)return;const m=d.dataset.pgAction;m==="prev"?I[t]=Math.max(1,I[t]-1):m==="next"?I[t]++:m==="goto"&&(I[t]=parseInt(d.dataset.pg)),t==="agents"?pt(!1):t==="sectors"?gt(!1):t==="airlines"?wt(!1):t==="reportFares"?jt(W):t==="databaseFares"&&z()}))}function Fe(t){var a,s;const e=!!t;dt(e?"Edit Agent":"Add New Agent",`
    ...
    </form>`),(a=document.getElementById("modal-cancel"))==null||a.addEventListener("click",()=>document.getElementById("admin-modal").close()),(s=document.getElementById("agent-form"))==null||s.addEventListener("submit",async i=>{i.preventDefault();const n=new FormData(i.target),l=Object.fromEntries(n.entries()),r=i.target.querySelector("[type=submit]");r.disabled=!0,r.textContent="Saving…";try{e?(await ea(t.id,l),$("success","Updated",`Agent "${l.name}" updated.`)):(await aa(l),$("success","Added",`Agent "${l.name}" added.`)),document.getElementById("admin-modal").close(),await pt()}catch(o){$("error","Save Failed",o.message),r.disabled=!1,r.textContent=e?"Save Changes":"Add Agent"}})}async function gt(t=!0){t&&(M=de(await oe()),I.sectors=1);const e=document.getElementById("sectors-search"),a=document.getElementById("sectors-limit");e&&!e.dataset.wired&&(e.dataset.wired="1",a.dataset.wired="1",e.addEventListener("input",m=>{Wt.sectors=m.target.value,I.sectors=1,gt(!1)}),a.addEventListener("change",m=>{J.sectors=parseInt(m.target.value),I.sectors=1,gt(!1)}));const s=document.querySelector("#sectors-tab .admin-table tbody");if(!s)return;const i=ut(M,"sectors"),n=J.sectors,l=Math.max(1,Math.ceil(i.length/n));I.sectors>l&&(I.sectors=l);const r=(I.sectors-1)*n,o=i.slice(r,r+n);s.innerHTML=o.length?o.map(m=>Va(m)).join(""):'<tr><td colspan="5" class="text-center py-8 text-text-muted">No sectors yet. Click "+ Add Sector".</td></tr>',Ct("sectors",i.length,l,r,n),za();const d=document.querySelector("#sectors-tab .flex.justify-between button");d&&!d.dataset.wired&&(d.dataset.wired="1",d.addEventListener("click",()=>je(null))),Ht("sectors")}function Va(t){const e=Le(t);return`<tr data-sector-id="${t.id}">
    <td class="font-mono text-xs text-text-muted">${t.id||"—"}</td>
    ...
    </form>`),(a=document.getElementById("modal-cancel"))==null||a.addEventListener("click",()=>document.getElementById("admin-modal").close()),(s=document.getElementById("sector-form"))==null||s.addEventListener("submit",async i=>{i.preventDefault();const n=new FormData(i.target),l=Object.fromEntries(n.entries());l.sectorCode=St(l.sectorCode.toUpperCase()),l.sectorFrom=St(l.sectorFrom.toUpperCase()),l.sectorTo=St(l.sectorTo.toUpperCase());const r=i.target.querySelector("[type=submit]");r.disabled=!0,r.textContent="Saving…";try{e?(await ia(t.id,l),$("success","Updated","Sector updated.")):(await oa(l),$("success","Added",`Sector "${l.sectorCode}" added.`)),document.getElementById("admin-modal").close(),await gt()}catch(o){$("error","Save Failed",o.message),r.disabled=!1,r.textContent=e?"Save Changes":"Add Sector"}})}async function wt(t=!0){t&&(O=await re(),I.airlines=1);const e=document.getElementById("airlines-search"),a=document.getElementById("airlines-limit");e&&!e.dataset.wired&&(e.dataset.wired="1",a.dataset.wired="1",e.addEventListener("input",m=>{Wt.airlines=m.target.value,I.airlines=1,wt(!1)}),a.addEventListener("change",m=>{J.airlines=parseInt(m.target.value),I.airlines=1,wt(!1)}));const s=document.querySelector("#flights-tab .admin-table tbody");if(!s)return;const i=ut(O,"airlines"),n=J.airlines,l=Math.max(1,Math.ceil(i.length/n));I.airlines>l&&(I.airlines=l);const r=(I.airlines-1)*n,o=i.slice(r,r+n);s.innerHTML=o.length?o.map(m=>Ga(m)).join(""):'<tr><td colspan="4" class="text-center py-8 text-text-muted">No airlines yet. Click "Add Airline".</td></tr>',Ct("airlines",i.length,l,r,n),Wa();const d=document.querySelector("#flights-tab .flex.justify-between button");d&&!d.dataset.wired&&(d.dataset.wired="1",d.addEventListener("click",()=>Re(null))),Ht("airlines")}function Ga(t){const e=t.logoUrl?`<span class="admin-logo-wrap"><img src="${t.logoUrl}" alt="${S(t.name||"Airline")}"></span>`:`<span class="admin-logo-wrap"><span class="admin-logo-fallback">${S((t.code||"NA").slice(0,3))}</span></span>`;return`<tr data-airline-id="${t.id}">
    <td>${e}</td>
    <td class="font-semibold">${t.name}</td>
  ...
  </tr>`}function Wa(){const t=document.querySelector("#flights-tab .admin-table tbody");!t||t.dataset.actionsWired||(t.dataset.actionsWired="1",t.addEventListener("click",async e=>{const a=e.target.closest("[data-action]");if(!a)return;const{action:s,id:i}=a.dataset,n=O.find(l=>l.id===i);if(s==="edit-airline"&&Re(n),s==="delete-airline"){if(!confirm(`Delete airline "${n==null?void 0:n.name}" (${n==null?void 0:n.code})?`))return;try{await ra(i),$("success","Deleted",`Airline "${n==null?void 0:n.name}" removed.`),await wt()}catch(l){$("error","Error",l.message)}}}))}function Re(t){var a,s;const e=!!t;dt(e?"Edit Airline":"Add New Airline",`
      ...
      <div class="flex gap-3 pt-2">
        ...
        <button type="button" id="modal-cancel" class="admin-btn admin-btn-ghost px-6 text-sm">Cancel</button>
    ...
    </form>`),(a=document.getElementById("modal-cancel"))==null||a.addEventListener("click",()=>document.getElementById("admin-modal").close()),(s=document.getElementById("airline-form"))==null||s.addEventListener("submit",async i=>{var d;i.preventDefault();const n=new FormData(i.target),l=((d=n.get("logoFile"))==null?void 0:d.size)>0?n.get("logoFile"):null,r={name:n.get("name"),code:n.get("code").toUpperCase()},o=i.target.querySelector("[type=submit]");o.disabled=!0,o.textContent="Saving…";try{e?(await da(t.id,r,l),$("success","Updated","Airline updated.")):(await la(r,l),$("success","Added",`Airline "${r.name}" added.`)),document.getElementById("admin-modal").close(),await wt()}catch(m){$("error","Save Failed",m.message),o.disabled=!1,o.textContent=e?"Save Changes":"Add Airline"}})}async function Ja(){const t=document.getElementById("reports-tab");if(!t||t.dataset.wired)return;t.dataset.wired="1";const e=document.getElementById("reports-sector-sel");e&&e.options.length<=1&&M.forEach(l=>e.appendChild(new Option(l.sectorCode,l.id)));const a=document.getElementById("reports-agent-sel");a&&a.options.length<=1&&Y.forEach(l=>a.appendChild(new Option(l.name,l.id)));const s=document.getElementById("generate-report-btn"),i=document.getElementById("reports-start-date"),n=document.getElementById("reports-end-date");s&&!s.dataset.wired&&(s.dataset.wired="1",s.addEventListener("click",async()=>{const l=(e==null?void 0:e.value)||"all",r=(a==null?void 0:a.value)||"all",o=(i==null?void 0:i.value)||null,d=(n==null?void 0:n.value)||null;s.disabled=!0,s.textContent="Generating…";try{const[m,c]=await Promise.all([ca(o,d,l,r),Gt({sectorId:l,agentId:r,startDate:o,endDate:d,includeHidden:!0})]);W=c,Ya(m,t),I.reportFares=1,jt(W)}catch(m){$("error","Report Failed",m.message)}finally{s.disabled=!1,s.innerHTML='<i class="bi bi-lightning-fill text-[13px]"></i> Generate Report'}}))}function Ya(t,e){const{agentReport:a,sectorReport:s,totalFares:i}=t,n=document.getElementById("report-stats-row");if(n){n.classList.remove("hidden");const x=(W||[]).filter(f=>!f.isHidden).length,u=(W||[]).filter(f=>f.isHidden).length,h=new Set((W||[]).map(f=>f.agentId)).size,E=(W||[]).map(f=>f.finalRate||0).filter(f=>f>0),p=E.length?Math.round(E.reduce((f,b)=>f+b,0)/E.length):0,v=(f,b)=>{const w=document.getElementById(f);w&&(w.textContent=b.toLocaleString())};v("stat-total-fares",i),v("stat-live-fares",x),v("stat-hidden-fares",u),v("stat-agents-count",h);const g=document.getElementById("stat-avg-fare");g&&(g.textContent=p>0?`₹${p.toLocaleString()}`:"—")}const l=document.getElementById("report-total-fares");l&&(l.textContent=`${i} fare${i!==1?"s":""} matched your filter`);const r=document.getElementById("bar-chart-container");r&&a.length&&Xa(a.slice(0,8),r);const o=document.getElementById("donut-chart-svg"),d=document.getElementById("pie-legend");o&&s.length&&Ka(s.slice(0,8),o,d);const m=document.getElementById("report-leaderboards");m&&(m.classList.remove("hidden"),Za(a,s));const c=document.getElementById("download-report-csv");if(c){const x=c.cloneNode(!0);c.parentNode.replaceChild(x,c),x.addEventListener("click",()=>Qa(W)),W&&W.length?x.classList.remove("opacity-50","pointer-events-none"):x.classList.add("opacity-50","pointer-events-none")}$("success","Report Ready",`${i} fare${i!==1?"s":""} aggregated.`)}function Xa(t,e){const a=e.clientWidth||480,s=260,i={top:32,right:16,bottom:48,left:48},n=a-i.left-i.right,l=s-i.top-i.bottom,r=Math.max(...t.map(f=>f.count),1),o=[["#0c4a8a","#3b82f6"],["#065f46","#22c55e"],["#78350f","#f59e0b"],["#7f1d1d","#ef4444"],["#4c1d95","#8b5cf6"],["#134e4a","#14b8a6"],["#7c2d12","#f97316"],["#1e293b","#64748b"]],d=4,m=Math.ceil(r/d),c=Array.from({length:d+1},(f,b)=>b*m),x=c.map(f=>{const b=i.top+l-f/(c[c.length-1]||1)*l;return`<line x1="${i.left}" y1="${b.toFixed(1)}" x2="${a-i.right}" y2="${b.toFixed(1)}" stroke="#f1f5f9" stroke-width="1"/>
            <text x="${i.left-6}" y="${(b+4).toFixed(1)}" text-anchor="end" font-size="10" fill="#94a3b8" font-weight="600">${f}</text>`}).join(""),u=Math.min(48,n/t.length*.6),h=n/t.length,E=t.map((f,b)=>{const w=Math.max(4,f.count/(c[c.length-1]||1)*l),B=i.left+b*h+h/2-u/2,T=i.top+l-w,[P,_]=o[b%o.length],k=`bg${b}`,H=f.avgRate?`avg ₹${Math.round(f.avgRate).toLocaleString()}`:"";return`<defs><linearGradient id="${k}" x1="0" y1="1" x2="0" y2="0">
            ...
            <g class="bar-group" data-name="${f.name}" data-count="${f.count}" data-avg="${H}" style="cursor:pointer;">
              <rect x="${B.toFixed(1)}" y="${T.toFixed(1)}" width="${u}" height="${w.toFixed(1)}"
                rx="6" fill="url(#${k})" opacity="0.92"
                style="transform-origin:${(B+u/2).toFixed(1)}px ${(i.top+l).toFixed(1)}px;
                       animation:barGrow 0.6s cubic-bezier(.34,1.56,.64,1) ${b*.07}s both;"/>
              <text x="${(B+u/2).toFixed(1)}" y="${(T-6).toFixed(1)}" text-anchor="middle"
                font-size="11" font-weight="900" fill="${_}">${f.count}</text>
              <text x="${(B+u/2).toFixed(1)}" y="${(i.top+l+16).toFixed(1)}" text-anchor="middle"
    ...
    <svg id="bar-svg" width="100%" height="${s}" viewBox="0 0 ${a} ${s}" style="overflow:visible;">
      ${x}
      <line x1="${i.left}" y1="${i.top}" x2="${i.left}" y2="${i.top+l}" stroke="#cbd5e1" stroke-width="1.5"/>
      <line x1="${i.left}" y1="${i.top+l}" x2="${a-i.right}" y2="${i.top+l}" stroke="#cbd5e1" stroke-width="1.5"/>
      ${E}
    </svg>`;const v=e.querySelector("#bar-svg"),g=e.querySelector(`#${p}`);v&&g&&v.querySelectorAll(".bar-group").forEach(f=>{f.addEventListener("mousemove",b=>{const w=e.getBoundingClientRect();g.style.display="block",g.style.left=b.clientX-w.left+12+"px",g.style.top=b.clientY-w.top-40+"px";const B=f.dataset.avg?`<br><span style="opacity:.7;font-weight:500;">${f.dataset.avg}</span>`:"";g.innerHTML=`${f.dataset.name}<br><span style="color:#60a5fa;">${f.dataset.count} fares</span>${B}`}),f.addEventListener("mouseleave",()=>{g.style.display="none"})})}function Ka(t,e,a){const s=["#1558c0","#3b82f6","#22c55e","#f59e0b","#ef4444","#8b5cf6","#14b8a6","#f97316"],o=t.reduce((f,b)=>f+b.count,0),d=e.getElementById?e.getElementById("donut-segments"):e.querySelector("#donut-segments"),m=e.querySelector("#donut-center-count"),c=e.querySelector("#donut-center-label");if(!d)return;m&&(m.textContent=o),c&&(c.textContent="FARES");const x=(f,b,w,B)=>({x:f+w*Math.cos((B-90)*Math.PI/180),y:b+w*Math.sin((B-90)*Math.PI/180)});let u=0;const h=t.map((f,b)=>{const w=o>0?f.count/o*360:0,B=u+w,T=w>180?1:0,P=x(110,110,95,u),_=x(110,110,95,B),k=x(110,110,60,u),H=x(110,110,60,B),Q=[`M ${P.x.toFixed(2)} ${P.y.toFixed(2)}`,`A 95 95 0 ${T} 1 ${_.x.toFixed(2)} ${_.y.toFixed(2)}`,`L ${H.x.toFixed(2)} ${H.y.toFixed(2)}`,`A 60 60 0 ${T} 0 ${k.x.toFixed(2)} ${k.y.toFixed(2)}`,"Z"].join(" "),V=u+w/2;u=B;const vt=o>0?(f.count/o*100).toFixed(1):"0.0";return{pathD:Q,color:s[b%s.length],name:f.name,count:f.count,pct:vt,mid:V}}),E="http://www.w3.org/2000/svg";d.innerHTML="";const p=h.map((f,b)=>{const w=document.createElementNS(E,"path");return w.setAttribute("d",f.pathD),w.setAttribute("fill",f.color),w.setAttribute("stroke","white"),w.setAttribute("stroke-width","2"),w.style.cursor="pointer",w.style.transition="transform 0.2s, filter 0.2s",w.style.transformOrigin="110px 110px",w.setAttribute("data-index",b),d.appendChild(w),w}),v=f=>{p.forEach((b,w)=>{w===f?(b.style.transform="scale(1.04)",b.style.filter="brightness(1.1)",b.setAttribute("stroke-width","3")):(b.style.transform="scale(1)",b.style.filter="brightness(1)",b.setAttribute("stroke-width","2"))}),f>=0&&f<h.length?(m&&(m.textContent=h[f].count),c&&(c.textContent=h[f].name.split(" ")[0].toUpperCase().slice(0,7))):(m&&(m.textContent=o),c&&(c.textContent="FARES"))};if(p.forEach((f,b)=>{f.addEventListener("mouseover",()=>{v(b),g(b)}),f.addEventListener("mouseout",()=>{v(-1),g(-1)})}),a){a.innerHTML=h.map((b,w)=>`
      ...
      </div>`).join("");const f=b=>{a.querySelectorAll(".legend-row").forEach((w,B)=>{w.style.background=B===b?"#f1f5f9":""})};window._highlightLegendRows=f,a.querySelectorAll(".legend-row").forEach((b,w)=>{b.addEventListener("mouseover",()=>{v(w),f(w)}),b.addEventListener("mouseout",()=>{v(-1),f(-1)})})}function g(f){window._highlightLegendRows&&window._highlightLegendRows(f)}}function Za(t,e){var n,l;const a=["#1558c0","#3b82f6","#22c55e","#f59e0b","#ef4444"],s=document.getElementById("leaderboard-agents");if(s&&t.length){const r=[...t].sort((d,m)=>m.count-d.count).slice(0,5),o=r[0].count||1;s.innerHTML=r.map((d,m)=>{const c=Math.max(6,Math.round(d.count/o*100));return`<div style="display:flex;align-items:center;gap:10px;">
      ...
      </div>`}).join("")}const i=document.getElementById("leaderboard-sectors");if(i&&e.length){const o=[...e.filter(c=>c.avgRate>0)].sort((c,x)=>c.avgRate-x.avgRate).slice(0,5),d=((n=o[0])==null?void 0:n.avgRate)||1,m=((l=o[o.length-1])==null?void 0:l.avgRate)||1;i.innerHTML=o.map((c,x)=>{const u=m>d?Math.max(6,Math.round((c.avgRate-d)/(m-d)*100)):50;return`<div style="display:flex;align-items:center;gap:10px;">
...
Please confirm availability!`;try{await navigator.clipboard.writeText(b),$("success","Copied!","Flight details copied to clipboard.")}catch(w){$("error","Copy failed",w.message)}return}if(l==="reset"){delete X[r],mt.delete(r),z();return}if(l==="delete"){if(!confirm("Delete this fare row? This cannot be undone."))return;n.disabled=!0;try{await ie(r),et=et.filter(o=>o.id!==r),delete X[r],nt.delete(r),mt.delete(r),$("success","Deleted","Fare row removed."),z()}catch(o){$("error","Delete Failed",o.message),n.disabled=!1}}}})}function sn(t){if(!t||t.dataset.controlsWired)return;t.dataset.controlsWired="1";const e=document.getElementById("database-search"),a=document.getElementById("database-agent-filter"),s=document.getElementById("database-sector-filter"),i=document.getElementById("database-airline-filter"),n=document.getElementById("database-status-filter"),l=document.getElementById("database-start-date"),r=document.getElementById("database-end-date"),o=document.getElementById("database-limit"),d=document.getElementById("database-clear-filters"),m=document.getElementById("database-refresh-btn"),c=document.getElementById("database-save-all-btn"),x=document.getElementById("database-delete-selected-btn"),u=document.getElementById("database-add-row-btn");e&&e.addEventListener("input",h=>{F.search=h.target.value||"",I.databaseFares=1,z()}),a&&a.addEventListener("change",h=>{F.agentId=h.target.value||"all",I.databaseFares=1,z()}),s&&s.addEventListener("change",h=>{F.sectorId=h.target.value||"all",I.databaseFares=1,z()}),i&&i.addEventListener("change",h=>{F.airlineId=h.target.value||"all",I.databaseFares=1,z()}),n&&n.addEventListener("change",h=>{F.status=h.target.value||"all",I.databaseFares=1,z()}),l&&l.addEventListener("change",h=>{F.startDate=h.target.value||"",I.databaseFares=1,z()}),r&&r.addEventListener("change",h=>{F.endDate=h.target.value||"",I.databaseFares=1,z()}),o&&(o.value=String(J.databaseFares),o.addEventListener("change",h=>{J.databaseFares=parseInt(h.target.value,10)||20,I.databaseFares=1,z()})),d&&d.addEventListener("click",()=>{F.search="",F.agentId="all",F.sectorId="all",F.airlineId="all",F.status="all",F.startDate="",F.endDate="",e&&(e.value=""),a&&(a.value="all"),s&&(s.value="all"),i&&(i.value="all"),n&&(n.value="all"),l&&(l.value=""),r&&(r.value=""),I.databaseFares=1,z()}),m&&m.addEventListener("click",async()=>{const h=m.innerHTML;m.disabled=!0,m.innerHTML='<i class="bi bi-arrow-repeat animate-spin"></i> Refreshing...',await ce(!0),m.disabled=!1,m.innerHTML=h}),c&&c.addEventListener("click",rn),x&&x.addEventListener("click",dn),u&&u.addEventListener("click",ln)}async function ce(t=!0){const e=document.getElementById("database-tab");if(!e)return;if(sn(e),nn(),an(),t||!e.dataset.loaded)try{et=await Gt({includeHidden:!0}),X={},nt=new Set,mt=new Set,I.databaseFares=1,e.dataset.loaded="1"}catch(s){$("error","Load Failed",s.message),et=[]}z()}function on(){const{agentNameById:t,sectorCodeById:e,airlineLabelById:a}=He(),s=F.search.trim().toLowerCase(),i=ja(F.startDate),n=Ra(F.endDate),l=et.map(d=>le(d)).filter(d=>{var x,u;if(F.agentId!=="all"&&d.agentId!==F.agentId||F.sectorId!=="all"&&d.sectorId!==F.sectorId||F.airlineId!=="all"&&d.airlineId!==F.airlineId||F.status==="live"&&d.isHidden||F.status==="hidden"&&!d.isHidden)return!1;const m=((u=(x=Mt(d.flightDate))==null?void 0:x.getTime)==null?void 0:u.call(x))||null;return i!==null&&(m===null||m<i)||n!==null&&(m===null||m>n)?!1:s?[d.id,Ft(d.flightDate),d.flightTime,d.specialRate,d.finalRate,d.commission,d.baggage,d.extraBaggage,d.isHidden?"hidden":"live",d.agentId,d.sectorId,d.airlineId,t[d.agentId]||"",e[d.sectorId]||"",a[d.airlineId]||""].join(" ").toLowerCase().includes(s):!0}),{key:r,asc:o}=rt.databaseFares;return l.sort((d,m)=>{const c=h=>{var E,p;return r==="agentId"?(t[h.agentId]||h.agentId||"").toLowerCase():r==="sectorId"?(e[h.sectorId]||h.sectorId||"").toLowerCase():r==="airlineId"?(a[h.airlineId]||h.airlineId||"").toLowerCase():r==="flightDate"?((p=(E=Mt(h.flightDate))==null?void 0:E.getTime)==null?void 0:p.call(E))||0:r==="isHidden"?h.isHidden?1:0:h[r]};let x=c(d),u=c(m);return typeof x=="string"&&(x=x.toLowerCase()),typeof u=="string"&&(u=u.toLowerCase()),x<u?o?-1:1:x>u?o?1:-1:0})}function z(){const t=document.getElementById("database-table-wrap");if(!t)return;const e=on(),{agentNameById:a,sectorCodeById:s,airlineLabelById:i,airlineCodeById:n}=He(),l=document.getElementById("database-total-count");l&&(l.textContent=e.length.toLocaleString());const r=J.databaseFares,o=Math.max(1,Math.ceil(e.length/r));I.databaseFares>o&&(I.databaseFares=o);const d=(I.databaseFares-1)*r,m=e.slice(d,d+r);if(!m.length){t.innerHTML=`<div class="admin-empty-state">
      <div class="admin-empty-state-card">
        <div class="admin-empty-state-icon">
    ...
    </div>`,Ct("databaseFares",e.length,o,d,r),Lt();return}const c=(p,v)=>`<th class="cursor-pointer group whitespace-nowrap" data-sort-tab="databaseFares" data-sort-key="${p}">
    ...
    <table class="admin-database-table">
      ...
      <tbody>
        ${m.map((p,v)=>{const g=!!X[p.id],f=nt.has(p.id),b=mt.has(p.id)||g,w=a[p.agentId]||p.agentId,B=s[p.sectorId]||p.sectorId;i[p.airlineId]||p.airlineId;const T=n[p.airlineId]||p.airlineId,P=p.flightDate instanceof Date?p.flightDate.toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"}):p.flightDate?Ft(p.flightDate):"—",_=v%2===1?"bg-slate-50/60":"";return`
            <tr data-fare-id="${p.id}" class="${g?"admin-database-row-dirty":_} hover:bg-slate-100/80 transition-colors">
              ...
              <td class="font-mono text-[11px] text-text-soft">${d+v+1}</td>
              ...
              <td class="whitespace-nowrap ${b?"":"text-[12px]"}">
                ...
                `:`<span class="bg-primary/10 text-primary font-bold px-2 py-0.5 rounded-md text-[12px]">${S(B)}</span>`}
              </td>
...
`)){const n=i.replace(/[*_~`]/g,"").trim();if(!n)continue;const l=n.match(/([A-Z]{3})\s+([A-Z]{3})/);if(l&&n.length<70&&!n.match(/\d{4,6}/)){a=l[1]+"-"+l[2];const r=n.match($e);r&&(s=r[1]);continue}if(a){const r=n.match($e);if(r&&!n.match(/\d{4,6}/)){s=r[1];continue}const o=n.match(/(\d{1,2})\s*(JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC).*?(\d{4,6})/i);if(o){const d=parseInt(o[3]);d>=1e3&&d<=99999&&e.push({sector:a,date:`2026-${mn[o[2].toUpperCase()]}-${o[1].padStart(2,"0")}`,airline:r?r[1]:s,rate:d})}}}return e}function gn(t){const e=Oe(t);if(!e.length){zt();return}const a=document.getElementById("prevBox");a&&a.classList.add("on");const s=document.getElementById("prevCount");s&&(s.textContent=e.length+" entr"+(e.length===1?"y":"ies"));const i=document.getElementById("prevBody");i&&(i.innerHTML=e.slice(0,60).map(n=>`
    ...
    <td class="px-4 py-2 text-sm text-center border-b border-border">₹${n.rate.toLocaleString()}</td></tr>`).join(""),e.length>60&&(i.innerHTML+=`<tr><td colspan="4" class="text-center p-3 text-xs text-text-muted">+ ${e.length-60} more</td></tr>`))}function zt(){var t;(t=document.getElementById("prevBox"))==null||t.classList.remove("on")}async function fn(){const t=document.getElementById("rateData");if(!K||!(t!=null&&t.value.trim()))return;const e=document.getElementById("submitBtn"),a=e.innerHTML;e.disabled=!0,e.innerHTML='<div class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> Sending to AI pipeline...';const s=document.getElementById("progBar"),i=document.getElementById("progFill");s&&s.classList.add("on");let n=0;const l=setInterval(()=>{n=Math.min(n+Math.random()*13,85),i&&(i.style.width=n+"%")},280),r=Oe(t.value),o={id:Date.now(),agent:K,time:new Date().toLocaleString("en-IN",{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"}),rows:r.length,status:"pen"};ft.unshift(o),ft.length>15&&ft.pop(),Ot(),Tt();try{const d=await fetch(cn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({agent_id:K,raw_text:t.value.trim(),timestamp:new Date().toISOString(),source:"zamra-portal"})});if(clearInterval(l),i&&(i.style.width="100%"),d.ok)o.status="ok",me+=r.length,Ot(),Tt(),se(),$("success","Submitted","Rates sent to the AI parser. Firestore will update in a moment."),setTimeout(()=>{t.value="";const m=document.getElementById("charCount");m&&(m.textContent="0 characters"),zt(),bt()},500);else throw new Error("N8N webhook rejected payload")}catch(d){clearInterval(l),i&&(i.style.width="100%"),o.status="err",Ot(),Tt(),$("error","Submission Failed",d.message)}setTimeout(()=>{s&&s.classList.remove("on"),i&&(i.style.width="0%"),e.innerHTML=a,bt()},900)}function se(){const t=document.getElementById("statSubs");t&&(t.textContent=ft.length);const e=document.getElementById("statEntries");e&&(e.textContent=me)}function Ot(){localStorage.setItem("zt_hist",JSON.stringify(ft))}function Tt(){const t=document.getElementById("historyWrap");if(t){if(!ft.length){t.innerHTML=`<div class="text-center py-6 text-text-muted text-sm bg-white/50 rounded-xl border border-dashed border-border flex flex-col items-center gap-2">
    ...
    </div>`}).join("")}}const bn=210/25.4*96,hn=297/25.4*96;function Ee(){const t=document.getElementById("eticket-output-wrapper"),e=document.getElementById("eticket-print-area");if(!t||!e||t.classList.contains("hidden"))return;e.style.zoom="1",e.style.removeProperty("--eticket-print-scale");const a=Math.max(e.scrollWidth,e.offsetWidth),s=Math.max(e.scrollHeight,e.offsetHeight);if(!a||!s)return;const i=bn/a,n=hn/s;let l=Math.min(1,i,n);l<1&&(l=Math.max(.7,l*.985)),e.style.zoom=String(l),e.style.setProperty("--eticket-print-scale",String(l))}function yn(){const t=document.getElementById("eticket-print-area");t&&(t.style.zoom="1",t.style.removeProperty("--eticket-print-scale"))}async function vn(){var r;const t=document.getElementById("eticket-tab");if(!t)return;const e=document.getElementById("eticket-form"),a=document.getElementById("et-add-passenger"),s=document.getElementById("et-passengers-container"),i=document.getElementById("et-airline"),n=document.getElementById("et-origin"),l=document.getElementById("et-destination");if(O.length===0&&(O=await re()),M.length===0&&(M=de(await oe())),!t.dataset.wired){if(t.dataset.wired="1",i&&O&&(i.innerHTML='<option value="">Select Airline</option>'+O.map(d=>`<option value="${d.name}">${d.name}</option>`).join("")),n&&M){const d=[...new Set(M.map(m=>m.sectorFrom).filter(Boolean))].sort();n.innerHTML='<option value="">Select Origin</option>'+d.map(m=>`<option value="${m}">${m}</option>`).join("")}if(l&&M){const d=[...new Set(M.map(m=>m.sectorTo).filter(Boolean))].sort();l.innerHTML='<option value="">Select Destination</option>'+d.map(m=>`<option value="${m}">${m}</option>`).join("")}const o=()=>{const d=Array.from(s.querySelectorAll(".et-pax-row"));d.forEach((m,c)=>{const x=m.querySelector(".et-passenger-index");x&&(x.textContent=`Passenger ${c+1}`);const u=m.querySelector(".et-remove-passenger");u&&(d.length<=1?(u.classList.add("opacity-40","pointer-events-none"),u.setAttribute("aria-disabled","true")):(u.classList.remove("opacity-40","pointer-events-none"),u.removeAttribute("aria-disabled")))})};a==null||a.addEventListener("click",()=>{const d=`
      ...
      `;s.insertAdjacentHTML("beforeend",d),o()}),s==null||s.addEventListener("click",d=>{var c;const m=d.target.closest(".et-remove-passenger");m&&((c=m.closest(".et-pax-row"))==null||c.remove(),o())}),s.children.length===0&&(a==null||a.click()),o(),e==null||e.addEventListener("submit",async d=>{d.preventDefault(),await xn(new FormData(e))}),(r=document.getElementById("et-print-btn"))==null||r.addEventListener("click",()=>{Ee(),requestAnimationFrame(()=>window.print())}),window.addEventListener("beforeprint",Ee),window.addEventListener("afterprint",yn),e==null||e.addEventListener("reset",()=>{setTimeout(()=>{var d;Array.from(s.children).forEach((m,c)=>{c>0&&m.remove()}),s.children.length===0&&(a==null||a.click()),o(),(d=document.getElementById("eticket-output-wrapper"))==null||d.classList.add("hidden")},10),$("info","Form Reset","The E-Ticket form has been cleared.")})}}async function xn(t){var ct,tt,G;const e=(ct=t.get("etPnr"))==null?void 0:ct.toUpperCase(),a=(tt=t.get("etAirline"))==null?void 0:tt.toUpperCase(),s=(G=t.get("etFlightNo"))==null?void 0:G.toUpperCase(),i=t.get("etDate"),n=t.get("etDepTime"),l=t.get("etArrTime"),r=t.get("etPhone"),o=(C="")=>String(C).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;"),d=C=>{const q=/^([01]?\d|2[0-3]):([0-5]\d)$/.exec(C||"");return q?Number(q[1])*60+Number(q[2]):null},m=(C="")=>C.replace(/[^A-Za-z]/g,"").toUpperCase().slice(0,3)||"---",c=C=>{const q=(C||"").trim();let D=q,j="";const Z=q.match(/^(.*?)\s*\((.*?)\)$/);return Z&&(D=Z[1].trim(),j=Z[2].trim()),{city:D,code:j}},x=c(t.get("etOrigin")),u=c(t.get("etDest")),h=t.get("etOrigin")||"—",E=t.get("etDest")||"—";let p="—";if(i){const C=new Date(i);if(!isNaN(C.getTime())){const q=["SUN","MON","TUE","WED","THU","FRI","SAT"],D=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"];p=`${q[C.getDay()]}, ${String(C.getDate()).padStart(2,"0")} ${D[C.getMonth()]} ${C.getFullYear()}`}}const v=C=>document.getElementById(C);let g=x.code,f=u.code,b=null;if(typeof M<"u"){if(b=M.find(C=>C.sectorFrom===h&&C.sectorTo===E),!b&&h){const C=M.find(q=>q.sectorFrom===h);C&&C.sectorCode&&(g=C.sectorCode.split(/[ -]+/)[0])}if(!b&&E){const C=M.find(q=>q.sectorTo===E);C&&C.sectorCode&&(f=C.sectorCode.split(/[ -]+/).pop())}}const w=(g||m(x.city)).toUpperCase(),B=(f||m(u.city)).toUpperCase(),T=`${w} - ${B}`,P=`${(x.city||h).toUpperCase()} to ${(u.city||E).toUpperCase()}`,_=(x.city||h).toUpperCase(),k=(u.city||E).toUpperCase(),H=d(n),Q=d(l);if(H!==null&&Q!==null){let C=Q-H;C<0&&(C+=24*60);const q=Math.floor(C/60),D=C%60;`${q}${String(D).padStart(2,"0")}`}v("t-pnr")&&(v("t-pnr").textContent=e||"—"),v("t-issued-by")&&(v("t-issued-by").textContent=a||"—"),v("t-customer-phone")&&(v("t-customer-phone").textContent=r||"—"),v("t-flight-code")&&(v("t-flight-code").textContent=s||"—"),v("t-travel-date")&&(v("t-travel-date").textContent=p||"—"),v("t-route-code")&&(v("t-route-code").textContent=T),v("t-route-long")&&(v("t-route-long").textContent=P);const V=new Date,vt=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],$t=`${String(V.getDate()).padStart(2,"0")} ${vt[V.getMonth()]} ${V.getFullYear()} ${String(V.getHours()).padStart(2,"0")}:${String(V.getMinutes()).padStart(2,"0")}`;v("t-booked-on")&&(v("t-booked-on").textContent=$t);const at=v("t-airline-logo"),st=v("t-issued-by-fallback");if(at){const C=typeof O<"u"?O.find(q=>q.name.toUpperCase()===a):null;C&&C.logoUrl?(at.src=C.logoUrl,at.classList.remove("hidden"),st&&st.classList.add("hidden")):(at.removeAttribute("src"),at.classList.add("hidden"),st&&(st.classList.remove("hidden"),st.textContent=(a||"No logo").toUpperCase()))}const y=t.getAll("paxTitle[]"),A=t.getAll("paxName[]"),L=t.getAll("paxType[]"),U=t.getAll("paxCheckBag[]"),N=t.getAll("paxCarryBag[]");v("t-pax-count")&&(v("t-pax-count").textContent=String(A.length)),v("t-top-pax-count")&&(v("t-top-pax-count").textContent=String(A.length));const Et=document.getElementById("t-passengers-tbody");if(Et){const C=A.map((q,D)=>{const j=o((y[D]||"MR").toUpperCase()),Z=o((A[D]||"").toUpperCase()),Pt=o((L[D]||"ADT").toUpperCase()),It=o(xe(U[D])),Xt=o(xe(N[D])),ot=b&&b.sectorCode?o(b.sectorCode.toUpperCase()):o(T);return`
    ...
    `}const it=document.getElementById("t-travel-tbody");it&&(it.innerHTML=`
      <tr class="text-slate-800">
        ...
        <td class="p-2.5 border-l border-t border-slate-200 align-top">
          ...
          <div class="text-[10px] text-slate-500 uppercase">${o(B)}</div>
          <div class="text-[13px] mt-1"><span class="font-bold">${o(l||"—")}</span> <span class="text-slate-500 ml-1 text-[11px]">${o(p||"—")}</span></div>
        </td>
    ...
    </button>`,s.appendChild(i),setTimeout(()=>i.isConnected&&i.remove(),7e3)}window.toast=$;document.addEventListener("DOMContentLoaded",()=>{});async function lt(t=!0){if(t)try{const[n,l,r,o]=await Promise.all([Ae(),ma(),ua(),pa()]);_t=n,Zt=l,Qt=r,te=o,I.visas=1,I.visaStampings=1,I.attestations=1,I.passportServices=1}catch(n){$("error","Error loading Visas tab data",n.message)}const e=document.querySelector("#visas-tab #visas-table-body");if(e){const n=ut(_t,"visas"),l=J.visas,r=Math.max(1,Math.ceil(n.length/l));I.visas>r&&(I.visas=r);const o=(I.visas-1)*l,d=n.slice(o,o+l);e.innerHTML=d.length?d.map(m=>$n(m)).join(""):'<tr><td colspan="5" class="text-center py-8 text-text-muted">No tourist visas yet. Click "Add Tourist Visa".</td></tr>',En()}const a=document.querySelector("#visa-stamping-table-body");if(a){const n=ut(Zt,"visaStampings"),l=J.visaStampings,r=Math.max(1,Math.ceil(n.length/l));I.visaStampings>r&&(I.visaStampings=r);const o=(I.visaStampings-1)*l,d=n.slice(o,o+l);a.innerHTML=d.length?d.map(m=>In(m)).join(""):'<tr><td colspan="4" class="text-center py-8 text-text-muted">No visa stampings yet. Click "Add Visa Stamping".</td></tr>',Sn()}const s=document.querySelector("#attestations-table-body");if(s){const n=ut(Qt,"attestations"),l=J.attestations,r=Math.max(1,Math.ceil(n.length/l));I.attestations>r&&(I.attestations=r);const o=(I.attestations-1)*l,d=n.slice(o,o+l);s.innerHTML=d.length?d.map(m=>Bn(m)).join(""):'<tr><td colspan="4" class="text-center py-8 text-text-muted">No attestations yet. Click "Add Attestation".</td></tr>',Cn()}const i=document.querySelector("#passport-services-table-body");if(i){const n=ut(te,"passportServices"),l=J.passportServices,r=Math.max(1,Math.ceil(n.length/l));I.passportServices>r&&(I.passportServices=r);const o=(I.passportServices-1)*l,d=n.slice(o,o+l);i.innerHTML=d.length?d.map(m=>An(m)).join(""):'<tr><td colspan="4" class="text-center py-8 text-text-muted">No passport services yet. Click "Add Passport Service".</td></tr>',Ln()}wn()}function wn(){const t=document.getElementById("visas-add-btn");t&&!t.dataset.wired&&(t.dataset.wired="1",t.addEventListener("click",()=>_e(null)));const e=document.getElementById("visa-stamping-add-btn");e&&!e.dataset.wired&&(e.dataset.wired="1",e.addEventListener("click",()=>Ve(null)));const a=document.getElementById("attestation-add-btn");a&&!a.dataset.wired&&(a.dataset.wired="1",a.addEventListener("click",()=>ze(null)));const s=document.getElementById("passport-service-add-btn");s&&!s.dataset.wired&&(s.dataset.wired="1",s.addEventListener("click",()=>Ge(null)))}function $n(t){const e=t.flagUrl?`<span class="admin-logo-wrap"><img src="${t.flagUrl}" alt="${S(t.countryName||"Country")}"></span>`:'<span class="admin-logo-wrap"><span class="admin-logo-fallback"><i class="bi bi-flag"></i></span></span>';return`<tr data-visa-id="${t.id}">
    <td class="w-16">${e}</td>
  ...
  </tr>`}function En(){const t=document.querySelector("#visas-tab .admin-table tbody");!t||t.dataset.actionsWired||(t.dataset.actionsWired="1",t.addEventListener("click",async e=>{const a=e.target.closest("[data-action]");if(!a)return;const{action:s,id:i}=a.dataset,n=_t.find(l=>l.id===i);if(s==="edit-visa"&&_e(n),s==="delete-visa"){if(!confirm(`Delete visa for "${n==null?void 0:n.countryName}"?`))return;try{await ga(i),$("success","Deleted",`Visa for "${n==null?void 0:n.countryName}" removed.`),await lt()}catch(l){$("error","Error",l.message)}}}))}function _e(t){const e=document.getElementById("modal-visa-form");if(!e)return;dt(t?"Edit Visa":"Add New Visa",e.innerHTML);const a=document.getElementById("visa-form"),s=document.getElementById("visa-id"),i=document.getElementById("visa-country"),n=document.getElementById("visa-type"),l=document.getElementById("visa-rate");t&&(s.value=t.id,i.value=t.countryName||"",n.value=t.visaType||"",l.value=t.rate||0),a.addEventListener("submit",async r=>{r.preventDefault();const o=a.querySelector('button[type="submit"]');o.disabled=!0,o.textContent="Saving...";try{const d=s.value,m={countryName:i.value.trim(),visaType:n.value.trim(),rate:Number(l.value)},x=document.getElementById("visa-flag").files[0];d?await fa(d,m,x):await ba(m,x),$("success","Saved!",`Visa for ${m.countryName} saved.`),document.getElementById("admin-modal").close(),await lt()}catch(d){$("error","Error",d.message),o.disabled=!1,o.textContent="Save Visa"}})}function In(t){return`<tr data-id="${t.id}">
  ...
  </tr>`}function Sn(){const t=document.getElementById("visa-stamping-table-body");!t||t.dataset.actionsWired||(t.dataset.actionsWired="1",t.addEventListener("click",async e=>{const a=e.target.closest("[data-action]");if(!a)return;const{action:s,id:i}=a.dataset,n=Zt.find(l=>l.id===i);if(s==="edit-visa-stamping"&&Ve(n),s==="delete-visa-stamping"){if(!confirm(`Delete visa stamping for "${n==null?void 0:n.country}"?`))return;try{await ha(i),$("success","Deleted",`Visa Stamping for "${n==null?void 0:n.country}" removed.`),await lt(!0)}catch(l){$("error","Error",l.message)}}}))}function Ve(t){const e=document.getElementById("modal-visa-stamping-form");if(!e)return;dt(t?"Edit Visa Stamping":"Add Visa Stamping",e.innerHTML);const a=document.getElementById("visa-stamping-form"),s=document.getElementById("visa-stamping-id"),i=document.getElementById("visa-stamping-country"),n=document.getElementById("visa-stamping-desc"),l=document.getElementById("visa-stamping-cost");t&&(s.value=t.id,i.value=t.country||"",n.value=t.description||"",l.value=t.cost||0),a.addEventListener("submit",async r=>{r.preventDefault();const o=a.querySelector('button[type="submit"]');o.disabled=!0,o.textContent="Saving...";try{const d=s.value,m={country:i.value.trim(),description:n.value.trim(),cost:Number(l.value)};d?await ya(d,m):await va(m),$("success","Saved!",`Visa stamping for ${m.country} saved.`),document.getElementById("admin-modal").close(),await lt(!0)}catch(d){$("error","Error",d.message),o.disabled=!1,o.textContent="Save"}})}function Bn(t){return`<tr data-id="${t.id}">
    <td class="font-bold text-navy">${S(t.country)}</td>
  ...
  </tr>`}function Cn(){const t=document.getElementById("attestations-table-body");!t||t.dataset.actionsWired||(t.dataset.actionsWired="1",t.addEventListener("click",async e=>{const a=e.target.closest("[data-action]");if(!a)return;const{action:s,id:i}=a.dataset,n=Qt.find(l=>l.id===i);if(s==="edit-attestation"&&ze(n),s==="delete-attestation"){if(!confirm(`Delete attestation for "${n==null?void 0:n.country}"?`))return;try{await xa(i),$("success","Deleted",`Attestation for "${n==null?void 0:n.country}" removed.`),await lt(!0)}catch(l){$("error","Error",l.message)}}}))}function ze(t){const e=document.getElementById("modal-attestation-form");if(!e)return;dt(t?"Edit Attestation":"Add Attestation",e.innerHTML);const a=document.getElementById("attestation-form"),s=document.getElementById("attestation-id"),i=document.getElementById("attestation-country"),n=document.getElementById("attestation-cert"),l=document.getElementById("attestation-cost");t&&(s.value=t.id,i.value=t.country||"",n.value=t.certificate||"",l.value=t.cost||0),a.addEventListener("submit",async r=>{r.preventDefault();const o=a.querySelector('button[type="submit"]');o.disabled=!0,o.textContent="Saving...";try{const d=s.value,m={country:i.value.trim(),certificate:n.value.trim(),cost:Number(l.value)};d?await wa(d,m):await $a(m),$("success","Saved!",`Attestation for ${m.country} saved.`),document.getElementById("admin-modal").close(),await lt(!0)}catch(d){$("error","Error",d.message),o.disabled=!1,o.textContent="Save"}})}function An(t){return`<tr data-id="${t.id}">
  ...
  </tr>`}function Ln(){const t=document.getElementById("passport-services-table-body");!t||t.dataset.actionsWired||(t.dataset.actionsWired="1",t.addEventListener("click",async e=>{const a=e.target.closest("[data-action]");if(!a)return;const{action:s,id:i}=a.dataset,n=te.find(l=>l.id===i);if(s==="edit-passport-service"&&Ge(n),s==="delete-passport-service"){if(!confirm(`Delete passport service "${n==null?void 0:n.type}"?`))return;try{await Ea(i),$("success","Deleted",`Passport service "${n==null?void 0:n.type}" removed.`),await lt(!0)}catch(l){$("error","Error",l.message)}}}))}function Ge(t){const e=document.getElementById("modal-passport-service-form");if(!e)return;dt(t?"Edit Passport Service":"Add Passport Service",e.innerHTML);const a=document.getElementById("passport-service-form"),s=document.getElementById("passport-service-id"),i=document.getElementById("passport-service-type"),n=document.getElementById("passport-service-desc"),l=document.getElementById("passport-service-cost");t&&(s.value=t.id,i.value=t.type||"",n.value=t.description||"",l.value=t.cost||0),a.addEventListener("submit",async r=>{r.preventDefault();const o=a.querySelector('button[type="submit"]');o.disabled=!0,o.textContent="Saving...";try{const d=s.value,m={type:i.value.trim(),description:n.value.trim(),cost:Number(l.value)};d?await Ia(d,m):await Sa(m),$("success","Saved!",`Passport service ${m.type} saved.`),document.getElementById("admin-modal").close(),await lt(!0)}catch(d){$("error","Error",d.message),o.disabled=!1,o.textContent="Save"}})}async function Jt(t=!0){if(t)try{ee=await Ba({includeInactive:!0}),I.tours=1}catch(r){$("error","Error loading Tours",r.message)}const e=document.getElementById("tours-table-body");if(!e)return;const a=ut(ee,"tours"),s=J.tours,i=Math.max(1,Math.ceil(a.length/s));I.tours>i&&(I.tours=i);const n=(I.tours-1)*s,l=a.slice(n,n+s);e.innerHTML=l.length?l.map(r=>kn(r)).join(""):'<tr><td colspan="7" class="text-center py-8 text-text-muted">No tour packages yet. Click "Add Tour Package".</td></tr>',Dn(),Tn()}function Tn(){const t=document.getElementById("tours-add-btn");t&&!t.dataset.wired&&(t.dataset.wired="1",t.addEventListener("click",()=>We(null)))}function kn(t){const e=t.coverImageUrl?`<span class="admin-logo-wrap"><img src="${t.coverImageUrl}" alt="${S(t.title)}" style="object-fit:cover;width:44px;height:36px;border-radius:6px;"></span>`:'<span class="admin-logo-wrap"><span class="admin-logo-fallback"><i class="bi bi-image"></i></span></span>',a=!t.price||t.price===0?'<span class="text-text-muted text-[12px] italic">Call for Price</span>':`<span class="font-black text-[15px] text-navy">₹${Number(t.price).toLocaleString()}</span>`,s=t.isActive!==!1?'<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-green-50 text-green-700 text-[11px] font-semibold"><i class="bi bi-check-circle-fill text-[9px]"></i>Active</span>':'<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 text-[11px] font-semibold"><i class="bi bi-dash-circle text-[9px]"></i>Hidden</span>';return`<tr data-tour-id="${t.id}">
    <td class="w-16">${e}</td>
  ...
  </tr>`}function Dn(){const t=document.getElementById("tours-table-body");!t||t.dataset.actionsWired||(t.dataset.actionsWired="1",t.addEventListener("click",async e=>{const a=e.target.closest("[data-action]");if(!a)return;const{action:s,id:i}=a.dataset,n=ee.find(l=>l.id===i);if(s==="edit-tour"&&We(n),s==="delete-tour"){if(!confirm(`Delete tour package "${n==null?void 0:n.title}"?`))return;try{await Ca(i),$("success","Deleted",`Tour "${n==null?void 0:n.title}" removed.`),await Jt()}catch(l){$("error","Error",l.message)}}}))}function kt(t=""){return t.split(`
...
`).map(n=>n.trim()).filter(Boolean)}}).filter(a=>a.day)}function We(t){const e=document.getElementById("modal-tour-form");if(!e)return;dt(t?"Edit Tour Package":"Add Tour Package",e.innerHTML,!0);const a=document.getElementById("tour-form"),s=document.getElementById("tour-id"),i=document.getElementById("tour-title"),n=document.getElementById("tour-category"),l=document.getElementById("tour-duration"),r=document.getElementById("tour-price"),o=document.getElementById("tour-active"),d=document.getElementById("tour-description"),m=document.getElementById("tour-highlights"),c=document.getElementById("tour-inclusions"),x=document.getElementById("tour-exclusions"),u=document.getElementById("tour-itinerary-container"),h=document.getElementById("tour-add-day-btn"),E=(p="",v=[])=>{const g=u.querySelectorAll(".tour-day-row").length;u.insertAdjacentHTML("beforeend",Mn(g,p,v)),Se(u)};h==null||h.addEventListener("click",()=>{var p;E(),(p=u.lastElementChild)==null||p.scrollIntoView({behavior:"smooth",block:"nearest"})}),u.addEventListener("click",p=>{var g;const v=p.target.closest(".tour-remove-day");v&&((g=v.closest(".tour-day-row"))==null||g.remove(),Se(u))}),t&&(s.value=t.id,i.value=t.title||"",n.value=t.category||"International",l.value=t.duration||"",r.value=t.price||0,o.checked=t.isActive!==!1,d.value=t.description||"",m.value=Dt(t.highlights),c.value=Dt(t.inclusions),x.value=Dt(t.exclusions),(Array.isArray(t.itinerary)?t.itinerary:[]).forEach(v=>E(v.day||"",v.activities||[]))),u.querySelectorAll(".tour-day-row").length===0&&E(),a.addEventListener("submit",async p=>{var g;p.preventDefault();const v=a.querySelector('button[type="submit"]');v.disabled=!0,v.textContent="Saving…";try{const f=s.value,b=Fn(u),w={title:i.value.trim(),category:n.value,duration:l.value.trim(),price:Number(r.value)||0,isActive:o.checked,description:d.value.trim(),highlights:kt(m.value),itinerary:b,inclusions:kt(c.value),exclusions:kt(x.value)},B=((g=document.getElementById("tour-image"))==null?void 0:g.files[0])||null;f?await Aa(f,w,B):await La(w,B),$("success","Saved!",`Tour "${w.title}" saved.`),document.getElementById("admin-modal").close(),await Jt()}catch(f){$("error","Error",f.message),v.disabled=!1,v.textContent="Save Tour"}})}async function Yt(t=!0){if(t)try{ae=await Ta({includeInactive:!0}),I.hajjUmrah=1}catch(r){$("error","Error loading Hajj & Umrah",r.message)}const e=document.getElementById("hajjumrah-table-body");if(!e)return;const a=ut(ae,"hajjUmrah"),s=J.hajjUmrah,i=Math.max(1,Math.ceil(a.length/s));I.hajjUmrah>i&&(I.hajjUmrah=i);const n=(I.hajjUmrah-1)*s,l=a.slice(n,n+s);e.innerHTML=l.length?l.map(r=>Rn(r)).join(""):'<tr><td colspan="10" class="text-center py-8 text-text-muted">No packages yet. Click "Add Package".</td></tr>',Nn(),jn()}function jn(){const t=document.getElementById("hajjumrah-add-btn");t&&!t.dataset.wired&&(t.dataset.wired="1",t.addEventListener("click",()=>Je(null)))}function Rn(t){const e=t.coverImageUrl?`<span class="admin-logo-wrap"><img src="${t.coverImageUrl}" alt="${S(t.title)}" style="object-fit:cover;width:44px;height:36px;border-radius:6px;"></span>`:'<span class="admin-logo-wrap"><span class="admin-logo-fallback"><i class="bi bi-image"></i></span></span>',a=!t.price||t.price===0?'<span class="text-text-muted text-[12px] italic">Call for Price</span>':`<span class="font-black text-[15px] text-navy">₹${Number(t.price).toLocaleString()}</span>`,s=t.isActive!==!1?'<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-green-50 text-green-700 text-[11px] font-semibold"><i class="bi bi-check-circle-fill text-[9px]"></i>Active</span>':'<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 text-[11px] font-semibold"><i class="bi bi-dash-circle text-[9px]"></i>Hidden</span>',i=t.type==="Hajj"?'<span class="px-2 py-0.5 rounded bg-blue-50 text-primary text-[11px] font-semibold">Hajj</span>':'<span class="px-2 py-0.5 rounded bg-amber-50 text-amber-600 text-[11px] font-semibold">Umrah</span>';return`<tr data-hajjumrah-id="${t.id}">
  ...
  </tr>`}function Nn(){const t=document.getElementById("hajjumrah-table-body");!t||t.dataset.actionsWired||(t.dataset.actionsWired="1",t.addEventListener("click",async e=>{const a=e.target.closest("[data-action]");if(!a)return;const{action:s,id:i}=a.dataset,n=ae.find(l=>l.id===i);if(s==="edit-hajjumrah"&&Je(n),s==="delete-hajjumrah"){if(!confirm(`Delete package "${n==null?void 0:n.title}"?`))return;try{await ka(i),$("success","Deleted",`Package "${n==null?void 0:n.title}" removed.`),await Yt()}catch(l){$("error","Error",l.message)}}}))}function Je(t){const e=document.getElementById("modal-hajjumrah-form");if(!e)return;dt(t?"Edit Package":"Add Package",e.innerHTML);const a=document.getElementById("hajjumrah-form"),s=document.getElementById("hajjumrah-id"),i=document.getElementById("hajjumrah-title"),n=document.getElementById("hajjumrah-type"),l=document.getElementById("hajjumrah-city"),r=document.getElementById("hajjumrah-airline"),o=document.getElementById("hajjumrah-date"),d=document.getElementById("hajjumrah-days"),m=document.getElementById("hajjumrah-nights"),c=document.getElementById("hajjumrah-price"),x=document.getElementById("hajjumrah-active"),u=document.getElementById("hajjumrah-description"),h=document.getElementById("hajjumrah-highlights"),E=document.getElementById("hajjumrah-inclusions");t&&(s.value=t.id,i.value=t.title||"",n.value=t.type||"Umrah",l.value=t.departureCity||"",r.value=t.airline||"",o.value=t.departureDate||"",d.value=t.days||15,m.value=t.nights||14,c.value=t.price||0,x.checked=t.isActive!==!1,u.value=t.description||"",h.value=Dt(t.highlights),E.value=Dt(t.inclusions)),a.addEventListener("submit",async p=>{var g;p.preventDefault();const v=a.querySelector('button[type="submit"]');v.disabled=!0,v.textContent="Saving…";try{const f=s.value,b={title:i.value.trim(),type:n.value,departureCity:l.value.trim(),airline:r.value.trim(),departureDate:o.value.trim(),days:Number(d.value)||1,nights:Number(m.value)||1,price:Number(c.value)||0,isActive:x.checked,description:u.value.trim(),highlights:kt(h.value),inclusions:kt(E.value)},w=((g=document.getElementById("hajjumrah-image"))==null?void 0:g.files[0])||null;f?await Da(f,b,w):await Ma(b,w),$("success","Saved!",`Package "${b.title}" saved.`),document.getElementById("admin-modal").close(),await Yt()}catch(f){$("error","Error",f.message),v.disabled=!1,v.textContent="Save Package"}})}
